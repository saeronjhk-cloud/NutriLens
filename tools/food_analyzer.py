...
### 절대 크기 레퍼런스 (사진에 있으면 ★최우선★ 활용)
**중요:** 사용자는 NutriLens의 정확도를 높이기 위해 일부러 아래 물체를 음식 옆에 두고 촬영합니다.
사진에 아래 reference 물체가 보이면, **반드시 그 물체의 길이를 기준으로** 음식의 실제 크기(지름·길이)를 역산하세요.
이 단계가 양 추정의 다른 모든 가이드보다 우선합니다.

[한국 표준 reference 길이]
- **숟가락 (한국 표준)**: 전체 길이 약 **20cm** (식당·집 평균값, 오차 ±10%)
- **젓가락 한 쌍 (한국 표준)**: 길이 약 **25cm** (오차 ±10%)
- **신용카드 (국제 표준 ISO/IEC 7810)**: 가로 **8.56cm**, 세로 5.4cm — 손으로 먹는 음식(피자/햄버거/김밥 등)에서 사용자가 의도적으로 함께 촬영
- 종이컵: 직경 7cm, 높이 9cm (200ml)
- 머그컵: 직경 8cm, 높이 9cm (300~350ml)
- 500ml 생수 페트병: 높이 약 21cm

[활용 방법 — 예시]
- 숟가락이 음식 옆에 보임 → "숟가락 길이 20cm를 기준으로 보니 피자 지름은 약 30cm" → 1조각 약 150g
- 신용카드가 햄버거 옆에 보임 → "카드 가로 8.56cm를 기준으로 햄버거 지름은 약 12cm" → 약 250g
- Reference 물체가 없으면 → 아래 [표준 식기 크기] 활용
