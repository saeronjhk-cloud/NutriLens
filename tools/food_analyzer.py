...
# ── AI 분석 프롬프트 (2026-05-06 칼로리 개선: 식기 표 + CoT + 다중음식 강화) ──
...
2. 각 음식의 **예상 양(g)**을 추정할 때 아래 [양 추정 가이드]를 따르세요
...
## [양 추정 가이드] — 4단계 추론 (Chain-of-Thought)

각 음식의 양을 추정할 때 머릿속으로 다음 4단계를 순서대로 거치세요:

  1단계 — 식기 식별: 사진의 식기/용기 종류와 크기 추정 (예: "지름 11cm 밥공기")
  2단계 — 채움 정도: 식기에서 음식이 차지하는 비율 (예: "밥공기의 70%")
  3단계 — 밀도 고려: 음식이 가벼운지(잎채소·뻥튀기) 무거운지(고기·밥·찌개) 판단
  4단계 — 최종 양(g): 위 3개를 종합해 g 단위 추정

### 절대 크기 레퍼런스 (사진에 ★실제로★ 보일 때만 사용)
**중요한 분기 조건:**
- 사진에 아래 reference 물체가 **명확히 보이면** → 그 물체의 표준 길이를 기준으로 음식 크기를 비교 추정
- 사진에 reference 물체가 **보이지 않으면** → 이 섹션은 **완전히 무시**하고 아래 [표준 식기 크기]·[음식별 표준 1인분]만 사용
- reference가 보이는지 애매하면 → 보이지 않는 것으로 간주 (보수적 판단)

[표준 reference 길이 — 정확도 등급별]

★ A급 (표준 명확, 오차 거의 0%)
- **신용카드/체크카드/교통카드/사원증** (ISO/IEC 7810 ID-1): 가로 **8.56cm**, 세로 5.4cm
- **명함** (한국 표준): 가로 **9cm**, 세로 5cm
- **500원 동전** (한국은행): 지름 **2.65cm**
- **100원 동전** (한국은행): 지름 **2.4cm**

▲ B급 (한국 표준 평균, 오차 ±10%)
- **숟가락** (한식): 길이 약 **20cm**
- **젓가락 한 쌍** (한식): 길이 약 **25cm**
- **양식 포크** (dinner fork): 길이 약 **20cm**
- **양식 나이프** (dinner knife): 길이 약 **22cm**
- **티스푼**: 길이 약 **13cm**

▲ B급 (컵·잔)
- 종이컵: 직경 7cm, 높이 9cm (200ml)
- 머그컵: 직경 8cm, 높이 9cm (300~350ml)
- 텀블러: 직경 7cm, 높이 약 17cm

▼ C급 (음료병)
- 500ml 생수 PET: 높이 약 **21cm**
- 1.5L 생수 PET: 높이 약 33cm
- 캔(355ml): 높이 약 12cm, 직경 6.5cm
- 캔(250ml): 높이 약 11cm, 직경 5.3cm

**활용 우선순위:** A급 reference가 보이면 그것을 우선 사용 (가장 정확).

**금지 사항:**
- reference가 사진에 없는데 "reference가 있다고 가정하고" 추정하지 말 것
- 추정한 크기를 reference 없이 "기억으로" 가져다 붙이지 말 것 (예: "보통 피자는 30cm" 같은 일반 지식만으로 reference 추정 결과처럼 사용 금지)

### 표준 식기 크기 (한국식)
- 밥공기: 직경 11cm, 깊이 5cm → 흰밥 1공기 약 210g
- 작은 반찬 종지: 직경 8cm → 반찬 30~50g
- 큰 사이드 그릇: 직경 13cm → 반찬 80~120g
- 큰 국·찌개 그릇: 직경 15cm, 깊이 7cm → 1인분 400~500g (국물 포함)
- 뚝배기 (소형): 직경 12~14cm → 찌개 350~450g
- 식판 한 칸: 약 100~200g 들어감
...
- 쿠키 1개: 15~30g
- 초콜릿 1조각: 30~50g
- 감자튀김 1인분: 100~150g (스몰), 200~250g (라지)

### 음료 (컵·캔 단위)
- 종이컵: 200g
- 머그컵: 350g
- 캔(355ml) / 작은 페트: 250~355g

## [다중 음식 처리 지침]

사진에 여러 음식이 함께 있을 때:
1. **가장 큰 기준 식기를 먼저 찾으세요** (예: 메인 접시, 밥공기 또는 큰 그릇)
2. 그 식기의 표준 크기를 기준으로 다른 반찬·사이드의 양을 **상대 면적 비율**로 추정하세요
3. 작은 종지에 담긴 반찬은 30~70g 범위로 보수적으로 추정하세요
...
# ── 음식 형태(shape) 자동 추론 — 칼로리 정확도 개선 (2026-05-06) ──
# Gemini 자문 반영: piece(개수)와 volume(부피) 음식의 양 추정 신뢰도가 다르므로
# 카테고리별로 차등 보정. CORE_FOODS 355건 일괄 수정 대신 키워드 기반 자동 분류.
#
# - piece: 개수·조각이 명확한 음식 (사과, 햄버거, 피자, 도넛, 김밥, 만두, 떡, 쿠키 등)
#          → AI 양 추정 100% 신뢰 (다이어터의 1/4 사과를 표준으로 강제하지 않음)
# - volume: 부피·국물 추정이 어려운 무정형 음식 (찌개, 국, 탕, 밥, 죽, 면, 볶음)
#          → AI 양과 CORE 표준이 크게 다르면 가중 평균 보정 (GPT 70% + CORE 30%)
# - drink: 음료 (컵·캔 단위) → 컵 표준 보정

# 무정형(부피 추정 어려움) 키워드
_VOLUME_KEYWORDS = (
    '찌개', '전골', '국', '탕', '죽', '면', '국수', '우동', '라면', '라멘',
    '짬뽕', '짜장', '카레', '리조또', '파스타', '스파게티', '뇨끼', '라자냐',
    '리비올리', '펜네', '링귀네', '스튜', '수프', '차우더',
    '볶음밥', '비빔밥', '덮밥', '잡채밥', '오므라이스', '제육덮밥',
    '볶음', '잡채', '나물', '무침', '조림',
)

# 음료 키워드
_DRINK_KEYWORDS = (
    '커피', '아메리카노', '라떼', '카푸치노', '카페모카', '바닐라라떼',
    '녹차', '홍차', '밀크티', '말차',
    '콜라', '사이다', '제로콜라', '주스', '쥬스',
    '우유', '저지방우유', '두유',
    '스무디', '에이드', '식혜',
...
def _detect_shape(food_name, category=None):
    """음식의 형태 자동 추론 — 'piece' | 'volume' | 'drink'.

    카테고리별 차등 보정의 기준이 됨. 휴리스틱이라 100% 정확하진 않지만
    신중한 키워드 우선순위로 약 90%+ 정확도 목표.

    우선순위:
      1. drink 키워드가 있으면 → 'drink'
      2. volume 키워드가 있으면 → 'volume'
      3. 그 외 → 'piece' (기본값, AI 추정 신뢰)
    """
    if not food_name:
        return 'piece'
...
CORE_FOODS = {
    ...
    # ── 1000종 확장: diet_fitness 추가 ──
    # 육류 단백질
    "소고기 채끝": {"cal":187,"prot":20.5,"carbs":0,"fat":11.2,"fiber":0,"sodium":59,"sugar":0,"serving":150,"category":"diet_fitness"},
    "소고기 우둔살": {"cal":131,"prot":21.8,"carbs":0,"fat":4.5,"fiber":0,"sodium":55,"sugar":0,"serving":150,"category":"diet_fitness"},
    "소고기 사태": {"cal":142,"prot":21.0,"carbs":0,"fat":6.2,"fiber":0,"sodium":52,"sugar":0,"serving":150,"category":"diet_fitness"},
    "소고기 홍두깨살": {"cal":118,"prot":22.0,"carbs":0,"fat":3.0,"fiber":0,"sodium":54,"sugar":0,"serving":150,"category":"diet_fitness"},
    "칠면조가슴살": {"cal":104,"prot":24.6,"carbs":0,"fat":0.7,"fiber":0,"sodium":50,"sugar":0,"serving":120,"category":"diet_fitness"},
    "칠면조햄": {"cal":126,"prot":18.0,"carbs":3.5,"fat":4.2,"fiber":0,"sodium":580,"sugar":2.0,"serving":60,"category":"diet_fitness"},
    "오리가슴살": {"cal":123,"prot":19.9,"carbs":0,"fat":4.5,"fiber":0,"sodium":65,"sugar":0,"serving":120,"category":"diet_fitness"},
    "양고기 등심": {"cal":162,"prot":20.4,"carbs":0,"fat":8.8,"fiber":0,"sodium":63,"sugar":0,"serving":150,"category":"diet_fitness"},
    "돼지 목살살코기": {"cal":180,"prot":19.5,"carbs":0,"fat":11.0,"fiber":0,"sodium":55,"sugar":0,"serving":150,"category":"diet_fitness"},
    "닭다리살": {"cal":177,"prot":18.2,"carbs":0,"fat":11.2,"fiber":0,"sodium":82,"sugar":0,"serving":120,"category":"diet_fitness"},
    "닭날개": {"cal":203,"prot":18.3,"carbs":0,"fat":14.3,"fiber":0,"sodium":77,"sugar":0,"serving":80,"category":"diet_fitness"},
    "닭가슴살캔": {"cal":110,"prot":23.0,"carbs":0.5,"fat":1.5,"fiber":0,"sodium":350,"sugar":0,"serving":90,"category":"diet_fitness"},
    "닭가슴살햄": {"cal":115,"prot":20.0,"carbs":3.0,"fat":2.5,"fiber":0,"sodium":520,"sugar":1.5,"serving":60,"category":"diet_fitness"},

    # 해산물 단백질
    "광어회": {"cal":103,"prot":20.0,"carbs":0,"fat":2.3,"fiber":0,"sodium":50,"sugar":0,"serving":100,"category":"diet_fitness"},
    "참치회": {"cal":132,"prot":23.3,"carbs":0,"fat":4.1,"fiber":0,"sodium":42,"sugar":0,"serving":100,"category":"diet_fitness"},
    "연어회": {"cal":183,"prot":20.1,"carbs":0,"fat":11.1,"fiber":0,"sodium":47,"sugar":0,"serving":100,"category":"diet_fitness"},
    "새우살": {"cal":85,"prot":20.1,"carbs":0,"fat":0.5,"fiber":0,"sodium":119,"sugar":0,"serving":80,"category":"diet_fitness"},
    "오징어회": {"cal":80,"prot":17.9,"carbs":0.8,"fat":0.4,"fiber":0,"sodium":198,"sugar":0,"serving":80,"category":"diet_fitness"},
    "문어숙회": {"cal":73,"prot":15.3,"carbs":2.1,"fat":0.4,"fiber":0,"sodium":230,"sugar":0,"serving":80,"category":"diet_fitness"},
    "바지락살": {"cal":72,"prot":12.8,"carbs":2.3,"fat":1.0,"fiber":0,"sodium":310,"sugar":0,"serving":60,"category":"diet_fitness"},
    "홍합살": {"cal":86,"prot":11.9,"carbs":3.7,"fat":2.2,"fiber":0,"sodium":286,"sugar":0,"serving":60,"category":"diet_fitness"},
    "대구살": {"cal":82,"prot":18.3,"carbs":0,"fat":0.7,"fiber":0,"sodium":54,"sugar":0,"serving":120,"category":"diet_fitness"},
    "도미회": {"cal":100,"prot":20.5,"carbs":0,"fat":1.7,"fiber":0,"sodium":53,"sugar":0,"serving":100,"category":"diet_fitness"},
    "가자미구이": {"cal":91,"prot":18.8,"carbs":0,"fat":1.5,"fiber":0,"sodium":78,"sugar":0,"serving":120,"category":"diet_fitness"},
    "꽁치구이": {"cal":171,"prot":20.6,"carbs":0,"fat":9.7,"fiber":0,"sodium":62,"sugar":0,"serving":100,"category":"diet_fitness"},
    "고등어구이": {"cal":190,"prot":20.3,"carbs":0,"fat":12.0,"fiber":0,"sodium":68,"sugar":0,"serving":100,"category":"diet_fitness"},
    "삼치구이": {"cal":155,"prot":21.5,"carbs":0,"fat":7.5,"fiber":0,"sodium":60,"sugar":0,"serving":100,"category":"diet_fitness"},

    # 식물성 단백질/콩류
    "템페": {"cal":192,"prot":20.3,"carbs":7.6,"fat":10.8,"fiber":0,"sodium":9,"sugar":0,"serving":80,"category":"diet_fitness"},
    "에다마메": {"cal":122,"prot":11.9,"carbs":8.9,"fat":5.2,"fiber":5.2,"sodium":6,"sugar":2.2,"serving":80,"category":"diet_fitness"},
    "렌틸콩": {"cal":116,"prot":9.0,"carbs":20.1,"fat":0.4,"fiber":7.9,"sodium":2,"sugar":1.8,"serving":80,"category":"diet_fitness"},
    "병아리콩": {"cal":164,"prot":8.9,"carbs":27.4,"fat":2.6,"fiber":7.6,"sodium":7,"sugar":4.8,"serving":80,"category":"diet_fitness"},
    "검은콩": {"cal":132,"prot":8.9,"carbs":23.7,"fat":0.5,"fiber":8.7,"sodium":1,"sugar":0.3,"serving":80,"category":"diet_fitness"},
    "흰강낭콩": {"cal":139,"prot":9.7,"carbs":25.1,"fat":0.5,"fiber":6.3,"sodium":6,"sugar":0.3,"serving":80,"category":"diet_fitness"},
    "완두콩": {"cal":81,"prot":5.4,"carbs":14.5,"fat":0.4,"fiber":5.1,"sodium":5,"sugar":5.7,"serving":80,"category":"diet_fitness"},
    "두부스테이크": {"cal":95,"prot":9.5,"carbs":3.0,"fat":5.2,"fiber":0.5,"sodium":180,"sugar":0.5,"serving":150,"category":"diet_fitness"},
    "순두부": {"cal":47,"prot":4.8,"carbs":1.6,"fat":2.5,"fiber":0.3,"sodium":140,"sugar":0.5,"serving":200,"category":"diet_fitness"},
    "연두부": {"cal":50,"prot":5.0,"carbs":2.0,"fat":2.5,"fiber":0.2,"sodium":5,"sugar":0.5,"serving":120,"category":"diet_fitness"},

    # 유제품/달걀
    "메추리알": {"cal":158,"prot":13.1,"carbs":0.4,"fat":11.1,"fiber":0,"sodium":141,"sugar":0.4,"serving":50,"category":"diet_fitness"},
    "달걀흰자": {"cal":52,"prot":10.9,"carbs":0.7,"fat":0.2,"fiber":0,"sodium":166,"sugar":0.7,"serving":100,"category":"diet_fitness"},
    "저지방치즈": {"cal":173,"prot":24.4,"carbs":1.9,"fat":7.0,"fiber":0,"sodium":600,"sugar":0.5,"serving":30,"category":"diet_fitness"},
    "모짜렐라치즈": {"cal":280,"prot":22.2,"carbs":2.2,"fat":20.7,"fiber":0,"sodium":627,"sugar":1.0,"serving":30,"category":"diet_fitness"},
    "스트링치즈": {"cal":280,"prot":25.0,"carbs":2.0,"fat":19.0,"fiber":0,"sodium":580,"sugar":1.0,"serving":28,"category":"diet_fitness"},
    "카제인단백질": {"cal":362,"prot":80.0,"carbs":4.0,"fat":1.5,"fiber":0,"sodium":200,"sugar":2.0,"serving":30,"category":"diet_fitness"},
    "유청단백질": {"cal":370,"prot":80.0,"carbs":7.0,"fat":2.0,"fiber":0,"sodium":180,"sugar":3.0,"serving":30,"category":"diet_fitness"},
    "저지방코티지치즈": {"cal":72,"prot":12.4,"carbs":2.7,"fat":1.0,"fiber":0,"sodium":330,"sugar":2.7,"serving":100,"category":"diet_fitness"},
    "스키르": {"cal":63,"prot":11.0,"carbs":3.5,"fat":0.2,"fiber":0,"sodium":50,"sugar":3.5,"serving":150,"category":"diet_fitness"},

    # 건강 곡물/탄수화물
    "현미": {"cal":350,"prot":7.4,"carbs":73.5,"fat":2.7,"fiber":3.4,"sodium":3,"sugar":0.7,"serving":70,"category":"diet_fitness"},
    "찰현미": {"cal":348,"prot":7.0,"carbs":74.2,"fat":2.3,"fiber":2.8,"sodium":3,"sugar":0.5,"serving":70,"category":"diet_fitness"},
    "귀리": {"cal":379,"prot":13.1,"carbs":67.7,"fat":6.5,"fiber":10.6,"sodium":6,"sugar":0.5,"serving":40,"category":"diet_fitness"},
    "율무": {"cal":360,"prot":13.0,"carbs":68.0,"fat":4.5,"fiber":1.6,"sodium":3,"sugar":0.5,"serving":70,"category":"diet_fitness"},
    "메밀": {"cal":343,"prot":13.3,"carbs":71.5,"fat":3.4,"fiber":10.0,"sodium":1,"sugar":0.5,"serving":70,"category":"diet_fitness"},
    "고구마": {"cal":86,"prot":1.6,"carbs":20.1,"fat":0.1,"fiber":3.0,"sodium":55,"sugar":4.2,"serving":150,"category":"diet_fitness"},
    "자색고구마": {"cal":90,"prot":1.4,"carbs":21.5,"fat":0.1,"fiber":3.2,"sodium":25,"sugar":5.0,"serving":150,"category":"diet_fitness"},
    "단호박죽": {"cal":48,"prot":1.0,"carbs":11.0,"fat":0.2,"fiber":1.2,"sodium":180,"sugar":4.5,"serving":250,"category":"diet_fitness"},
    "퀴노아샐러드": {"cal":106,"prot":4.0,"carbs":17.0,"fat":2.5,"fiber":2.5,"sodium":180,"sugar":1.5,"serving":200,"category":"diet_fitness"},
    "곤약쌀밥": {"cal":10,"prot":0.1,"carbs":2.3,"fat":0,"fiber":2.0,"sodium":15,"sugar":0,"serving":200,"category":"diet_fitness"},
    "키토빵": {"cal":210,"prot":12.0,"carbs":8.0,"fat":15.0,"fiber":5.0,"sodium":280,"sugar":1.0,"serving":60,"category":"diet_fitness"},
    "저칼로리빵": {"cal":200,"prot":10.0,"carbs":25.0,"fat":6.0,"fiber":8.0,"sodium":300,"sugar":2.0,"serving":50,"category":"diet_fitness"},

    # 저칼로리 대체식품
    "곤약국수": {"cal":8,"prot":0.1,"carbs":1.8,"fat":0,"fiber":2.5,"sodium":10,"sugar":0,"serving":200,"category":"diet_fitness"},
    "두부면": {"cal":60,"prot":6.5,"carbs":2.0,"fat":3.0,"fiber":0.5,"sodium":20,"sugar":0.5,"serving":150,"category":"diet_fitness"},
    "저칼로리아이스크림": {"cal":100,"prot":5.0,"carbs":18.0,"fat":2.0,"fiber":4.0,"sodium":80,"sugar":6.0,"serving":100,"category":"diet_fitness"},
    "저칼로리잼": {"cal":105,"prot":0.3,"carbs":26.0,"fat":0.1,"fiber":0.5,"sodium":15,"sugar":8.0,"serving":20,"category":"diet_fitness"},
    "단백질칩": {"cal":400,"prot":30.0,"carbs":40.0,"fat":14.0,"fiber":3.0,"sodium":600,"sugar":2.0,"serving":30,"category":"diet_fitness"},
    "단백질쿠키": {"cal":380,"prot":25.0,"carbs":35.0,"fat":15.0,"fiber":3.0,"sodium":250,"sugar":5.0,"serving":40,"category":"diet_fitness"},
    "단백질브라우니": {"cal":350,"prot":20.0,"carbs":38.0,"fat":13.0,"fiber":4.0,"sodium":200,"sugar":8.0,"serving":50,"category":"diet_fitness"},
    "단백질팬케이크": {"cal":160,"prot":20.0,"carbs":15.0,"fat":3.0,"fiber":2.0,"sodium":300,"sugar":3.0,"serving":100,"category":"diet_fitness"},
    "저칼로리소스": {"cal":30,"prot":0.5,"carbs":7.0,"fat":0,"fiber":0.3,"sodium":400,"sugar":3.0,"serving":30,"category":"diet_fitness"},
    "제로칼로리젤리": {"cal":0,"prot":0,"carbs":0,"fat":0,"fiber":0,"sodium":10,"sugar":0,"serving":100,"category":"diet_fitness"},
    "라이스페이퍼": {"cal":333,"prot":0.8,"carbs":82.3,"fat":0.2,"fiber":0.3,"sodium":75,"sugar":0.5,"serving":15,"category":"diet_fitness"},
    "월남쌈": {"cal":80,"prot":3.5,"carbs":14.0,"fat":1.0,"fiber":1.5,"sodium":200,"sugar":2.0,"serving":200,"category":"diet_fitness"},

    # 견과류/시드 추가
    "브라질너트": {"cal":659,"prot":14.3,"carbs":11.7,"fat":67.1,"fiber":7.5,"sodium":3,"sugar":2.3,"serving":15,"category":"diet_fitness"},
    "마카다미아": {"cal":718,"prot":7.9,"carbs":13.8,"fat":75.8,"fiber":8.6,"sodium":5,"sugar":4.6,"serving":15,"category":"diet_fitness"},
    "해바라기씨": {"cal":584,"prot":20.8,"carbs":20.0,"fat":51.5,"fiber":8.6,"sodium":9,"sugar":2.6,"serving":20,"category":"diet_fitness"},
    "호박씨": {"cal":559,"prot":30.2,"carbs":10.7,"fat":49.1,"fiber":6.0,"sodium":7,"sugar":1.4,"serving":20,"category":"diet_fitness"},
    "아마씨가루": {"cal":534,"prot":18.3,"carbs":28.9,"fat":42.2,"fiber":27.3,"sodium":30,"sugar":1.6,"serving":15,"category":"diet_fitness"},
    "헴프씨드": {"cal":553,"prot":31.6,"carbs":8.7,"fat":48.8,"fiber":4.0,"sodium":5,"sugar":1.5,"serving":15,"category":"diet_fitness"},

    # 기능성 음료/보충
    "전해질음료": {"cal":10,"prot":0,"carbs":2.5,"fat":0,"fiber":0,"sodium":200,"sugar":2.0,"serving":500,"category":"diet_fitness"},
    "콜라겐음료": {"cal":15,"prot":3.0,"carbs":1.0,"fat":0,"fiber":0,"sodium":15,"sugar":0.5,"serving":100,"category":"diet_fitness"},
    "MCT오일": {"cal":900,"prot":0,"carbs":0,"fat":100,"fiber":0,"sodium":0,"sugar":0,"serving":10,"category":"diet_fitness"},
    "코코넛오일": {"cal":862,"prot":0,"carbs":0,"fat":100,"fiber":0,"sodium":0,"sugar":0,"serving":10,"category":"diet_fitness"},
    "올리브오일": {"cal":884,"prot":0,"carbs":0,"fat":100,"fiber":0,"sodium":2,"sugar":0,"serving":10,"category":"diet_fitness"},
    "프로틴워터": {"cal":10,"prot":2.0,"carbs":0.5,"fat":0,"fiber":0,"sodium":15,"sugar":0,"serving":500,"category":"diet_fitness"},
    "식물성프로틴": {"cal":355,"prot":75.0,"carbs":10.0,"fat":3.0,"fiber":5.0,"sodium":300,"sugar":2.0,"serving":30,"category":"diet_fitness"},
    "분리유청단백질": {"cal":375,"prot":90.0,"carbs":2.0,"fat":1.0,"fiber":0,"sodium":150,"sugar":1.0,"serving":30,"category":"diet_fitness"},
    "소이프로틴": {"cal":338,"prot":80.7,"carbs":0.5,"fat":3.4,"fiber":0,"sodium":900,"sugar":0,"serving":30,"category":"diet_fitness"},

    # ── 1000종 확장: fruit 추가 ──
    "청포도": {"cal":69,"prot":0.7,"carbs":18.1,"fat":0.2,"fiber":0.9,"sodium":2,"sugar":15.5,"serving":100,"category":"fruit"},
    "적포도": {"cal":69,"prot":0.7,"carbs":18.1,"fat":0.2,"fiber":0.9,"sodium":2,"sugar":15.5,"serving":100,"category":"fruit"},
    "샤인머스캣": {"cal":69,"prot":0.7,"carbs":17.0,"fat":0.2,"fiber":0.9,"sodium":2,"sugar":14.0,"serving":120,"category":"fruit"},
    "천도복숭아": {"cal":44,"prot":1.1,"carbs":10.6,"fat":0.3,"fiber":1.7,"sodium":0,"sugar":7.9,"serving":150,"category":"fruit"},
    "황도": {"cal":39,"prot":0.9,"carbs":9.5,"fat":0.3,"fiber":1.5,"sodium":0,"sugar":8.4,"serving":150,"category":"fruit"},
    "아보카도": {"cal":160,"prot":2.0,"carbs":8.5,"fat":14.7,"fiber":6.7,"sodium":7,"sugar":0.7,"serving":70,"category":"fruit"},
    "백향과": {"cal":97,"prot":2.2,"carbs":23.4,"fat":0.7,"fiber":10.4,"sodium":28,"sugar":11.2,"serving":50,"category":"fruit"},
    "용과": {"cal":50,"prot":1.1,"carbs":11.0,"fat":0.4,"fiber":3.0,"sodium":39,"sugar":8.0,"serving":150,"category":"fruit"},
    "석류": {"cal":83,"prot":1.7,"carbs":18.7,"fat":1.2,"fiber":4.0,"sodium":3,"sugar":13.7,"serving":100,"category":"fruit"},
    "무화과": {"cal":74,"prot":0.8,"carbs":19.2,"fat":0.3,"fiber":2.9,"sodium":1,"sugar":16.3,"serving":80,"category":"fruit"},
    "리치": {"cal":66,"prot":0.8,"carbs":16.5,"fat":0.4,"fiber":1.3,"sodium":1,"sugar":15.2,"serving":80,"category":"fruit"},
    "구아바": {"cal":68,"prot":2.6,"carbs":14.3,"fat":1.0,"fiber":5.4,"sodium":2,"sugar":8.9,"serving":100,"category":"fruit"},
    "코코넛": {"cal":354,"prot":3.3,"carbs":15.2,"fat":33.5,"fiber":9.0,"sodium":20,"sugar":6.2,"serving":40,"category":"fruit"},
    "유자": {"cal":53,"prot":0.8,"carbs":13.3,"fat":0.3,"fiber":1.8,"sodium":3,"sugar":7.0,"serving":80,"category":"fruit"},
    "매실": {"cal":28,"prot":0.7,"carbs":6.9,"fat":0.2,"fiber":0.5,"sodium":2,"sugar":5.0,"serving":50,"category":"fruit"},
    "크랜베리": {"cal":46,"prot":0.4,"carbs":12.2,"fat":0.1,"fiber":4.6,"sodium":2,"sugar":4.0,"serving":50,"category":"fruit"},
    "대추": {"cal":277,"prot":1.8,"carbs":75.0,"fat":0.2,"fiber":6.7,"sodium":1,"sugar":63.4,"serving":30,"category":"fruit"},
    "건포도": {"cal":299,"prot":3.1,"carbs":79.2,"fat":0.5,"fiber":3.7,"sodium":11,"sugar":59.2,"serving":30,"category":"fruit"},
    "냉동딸기": {"cal":35,"prot":0.7,"carbs":7.7,"fat":0.3,"fiber":2.0,"sodium":1,"sugar":4.9,"serving":100,"category":"fruit"},
    "냉동블루베리": {"cal":57,"prot":0.7,"carbs":14.5,"fat":0.3,"fiber":2.4,"sodium":1,"sugar":9.7,"serving":100,"category":"fruit"},
    "냉동망고": {"cal":60,"prot":0.8,"carbs":15.0,"fat":0.4,"fiber":1.6,"sodium":2,"sugar":13.7,"serving":100,"category":"fruit"},
    "건바나나칩": {"cal":519,"prot":2.3,"carbs":58.4,"fat":33.6,"fiber":7.7,"sodium":3,"sugar":35.3,"serving":30,"category":"fruit"},
    "플럼": {"cal":46,"prot":0.7,"carbs":11.4,"fat":0.3,"fiber":1.4,"sodium":0,"sugar":9.9,"serving":100,"category":"fruit"},

    # ── 1000종 확장: korean 보강 ──
    # 반찬류
    "멸치볶음(꽈리)": {"cal":180,"prot":22.0,"carbs":10.0,"fat":6.5,"fiber":1.0,"sodium":650,"sugar":5.0,"serving":30,"category":"korean"},
    "어묵볶음(간장)": {"cal":130,"prot":9.0,"carbs":12.0,"fat":5.0,"fiber":0.5,"sodium":580,"sugar":4.0,"serving":60,"category":"korean"},
    "김치볶음": {"cal":60,"prot":2.5,"carbs":8.0,"fat":2.0,"fiber":2.5,"sodium":450,"sugar":2.0,"serving":80,"category":"korean"},
    "무말랭이무침": {"cal":50,"prot":1.5,"carbs":10.5,"fat":0.5,"fiber":3.0,"sodium":400,"sugar":5.0,"serving":50,"category":"korean"},
    "연근조림": {"cal":115,"prot":2.0,"carbs":25.0,"fat":0.5,"fiber":2.0,"sodium":380,"sugar":12.0,"serving":50,"category":"korean"},
    "우엉조림": {"cal":120,"prot":2.5,"carbs":24.0,"fat":1.5,"fiber":3.0,"sodium":400,"sugar":10.0,"serving":50,"category":"korean"},
    "콩자반": {"cal":170,"prot":10.0,"carbs":22.0,"fat":4.5,"fiber":3.5,"sodium":500,"sugar":12.0,"serving":40,"category":"korean"},
    "마늘종볶음": {"cal":80,"prot":2.5,"carbs":12.0,"fat":2.5,"fiber":2.0,"sodium":350,"sugar":3.0,"serving":50,"category":"korean"},
    "고추장아찌": {"cal":75,"prot":1.8,"carbs":15.0,"fat":0.5,"fiber":2.0,"sodium":500,"sugar":5.0,"serving":30,"category":"korean"},
    "깻잎무침": {"cal":55,"prot":3.0,"carbs":7.0,"fat":2.0,"fiber":3.0,"sodium":320,"sugar":1.5,"serving":30,"category":"korean"},
    "미역줄기무침": {"cal":25,"prot":1.5,"carbs":4.0,"fat":0.5,"fiber":2.0,"sodium":300,"sugar":0.5,"serving":50,"category":"korean"},
    "도라지무침": {"cal":55,"prot":2.0,"carbs":10.0,"fat":1.0,"fiber":3.0,"sodium":280,"sugar":2.0,"serving":50,"category":"korean"},
    "고사리나물": {"cal":35,"prot":2.5,"carbs":5.0,"fat":1.0,"fiber":3.5,"sodium":250,"sugar":0.5,"serving":50,"category":"korean"},
    "취나물": {"cal":30,"prot":2.0,"carbs":4.5,"fat":0.8,"fiber":3.0,"sodium":200,"sugar":0.5,"serving":50,"category":"korean"},
    "묵무침": {"cal":35,"prot":0.5,"carbs":8.0,"fat":0.3,"fiber":0.5,"sodium":250,"sugar":0.5,"serving":100,"category":"korean"},
    "두부조림": {"cal":85,"prot":7.0,"carbs":6.0,"fat":4.0,"fiber":0.5,"sodium":400,"sugar":3.0,"serving":100,"category":"korean"},
    "장조림": {"cal":140,"prot":18.0,"carbs":8.0,"fat":4.0,"fiber":0,"sodium":600,"sugar":5.0,"serving":50,"category":"korean"},
    "메추리알장조림": {"cal":160,"prot":13.0,"carbs":9.0,"fat":8.0,"fiber":0,"sodium":580,"sugar":5.0,"serving":50,"category":"korean"},
    "호박볶음": {"cal":40,"prot":1.5,"carbs":5.0,"fat":1.5,"fiber":1.0,"sodium":250,"sugar":2.0,"serving":70,"category":"korean"},
    "버섯볶음": {"cal":35,"prot":2.5,"carbs":4.0,"fat":1.5,"fiber":1.5,"sodium":220,"sugar":1.5,"serving":70,"category":"korean"},
    "건새우볶음": {"cal":200,"prot":35.0,"carbs":10.0,"fat":2.5,"fiber":0.5,"sodium":700,"sugar":4.0,"serving":20,"category":"korean"},
    "오이무침": {"cal":25,"prot":1.0,"carbs":4.0,"fat":0.5,"fiber":1.0,"sodium":280,"sugar":2.0,"serving":60,"category":"korean"},
    "양배추쌈": {"cal":25,"prot":1.3,"carbs":5.8,"fat":0.1,"fiber":2.5,"sodium":18,"sugar":3.2,"serving":100,"category":"korean"},
    "상추쌈": {"cal":15,"prot":1.4,"carbs":2.5,"fat":0.2,"fiber":1.3,"sodium":5,"sugar":0.8,"serving":50,"category":"korean"},
    "깻잎쌈": {"cal":42,"prot":3.7,"carbs":5.0,"fat":1.3,"fiber":4.0,"sodium":3,"sugar":0.5,"serving":15,"category":"korean"},

    # 국/탕/찌개
    "사골국": {"cal":30,"prot":3.5,"carbs":0.5,"fat":1.5,"fiber":0,"sodium":350,"sugar":0,"serving":400,"category":"korean"},
    "소머리국밥": {"cal":55,"prot":7.0,"carbs":1.0,"fat":2.5,"fiber":0,"sodium":400,"sugar":0,"serving":400,"category":"korean"},
    "뼈해장국": {"cal":45,"prot":5.0,"carbs":2.0,"fat":2.0,"fiber":0.5,"sodium":420,"sugar":0.5,"serving":400,"category":"korean"},
    "콩나물해장국": {"cal":25,"prot":2.5,"carbs":2.5,"fat":0.5,"fiber":1.0,"sodium":380,"sugar":0.5,"serving":400,"category":"korean"},
    "황태국": {"cal":20,"prot":3.0,"carbs":1.0,"fat":0.3,"fiber":0.2,"sodium":350,"sugar":0.5,"serving":400,"category":"korean"},
    "무국": {"cal":15,"prot":1.0,"carbs":2.5,"fat":0.3,"fiber":0.5,"sodium":300,"sugar":1.0,"serving":400,"category":"korean"},
    "시래기된장국": {"cal":25,"prot":2.0,"carbs":3.0,"fat":0.8,"fiber":1.5,"sodium":380,"sugar":0.5,"serving":400,"category":"korean"},
    "북어국": {"cal":22,"prot":3.5,"carbs":1.0,"fat":0.5,"fiber":0.2,"sodium":360,"sugar":0.3,"serving":400,"category":"korean"},
    "조개탕": {"cal":25,"prot":3.5,"carbs":1.5,"fat":0.5,"fiber":0,"sodium":340,"sugar":0.3,"serving":400,"category":"korean"},
    "매운탕": {"cal":40,"prot":5.0,"carbs":3.0,"fat":1.0,"fiber":0.5,"sodium":400,"sugar":1.0,"serving":400,"category":"korean"},
    "추어탕": {"cal":55,"prot":6.0,"carbs":3.0,"fat":2.0,"fiber":0.5,"sodium":420,"sugar":0.5,"serving":400,"category":"korean"},
    "갈비해장국": {"cal":60,"prot":6.5,"carbs":2.0,"fat":3.0,"fiber":0.3,"sodium":430,"sugar":0.5,"serving":400,"category":"korean"},
    "닭곰탕": {"cal":35,"prot":4.5,"carbs":1.0,"fat":1.5,"fiber":0,"sodium":350,"sugar":0,"serving":400,"category":"korean"},
    "동태찌개": {"cal":30,"prot":3.5,"carbs":2.5,"fat":0.8,"fiber":0.5,"sodium":380,"sugar":0.5,"serving":400,"category":"korean"},
    "해물찌개": {"cal":35,"prot":4.0,"carbs":3.0,"fat":1.0,"fiber":0.5,"sodium":400,"sugar":0.5,"serving":400,"category":"korean"},
    "된장국": {"cal":20,"prot":1.5,"carbs":2.0,"fat":0.8,"fiber":0.5,"sodium":350,"sugar":0.5,"serving":400,"category":"korean"},
    "아욱국": {"cal":18,"prot":1.5,"carbs":2.5,"fat":0.3,"fiber":1.0,"sodium":300,"sugar":0.5,"serving":400,"category":"korean"},
    "잔치국수국물": {"cal":15,"prot":1.5,"carbs":1.5,"fat":0.3,"fiber":0,"sodium":350,"sugar":0.5,"serving":400,"category":"korean"},

    # 구이/볶음
    "제육덮밥(저염)": {"cal":130,"prot":10.0,"carbs":12.0,"fat":5.0,"fiber":0.5,"sodium":350,"sugar":3.0,"serving":250,"category":"korean"},
    "소불고기": {"cal":155,"prot":18.0,"carbs":7.0,"fat":6.5,"fiber":0.5,"sodium":420,"sugar":4.0,"serving":150,"category":"korean"},
    "닭갈비구이": {"cal":140,"prot":16.0,"carbs":8.0,"fat":5.0,"fiber":0.5,"sodium":380,"sugar":4.0,"serving":150,"category":"korean"},
    "삼겹살구이": {"cal":330,"prot":16.5,"carbs":0,"fat":29.5,"fiber":0,"sodium":65,"sugar":0,"serving":150,"category":"korean"},
    "항정살구이": {"cal":285,"prot":17.0,"carbs":0,"fat":24.0,"fiber":0,"sodium":60,"sugar":0,"serving":120,"category":"korean"},
    "소갈비구이": {"cal":220,"prot":19.0,"carbs":5.0,"fat":13.5,"fiber":0,"sodium":350,"sugar":3.0,"serving":150,"category":"korean"},
    "돼지불백": {"cal":175,"prot":16.0,"carbs":8.0,"fat":9.0,"fiber":0.5,"sodium":450,"sugar":5.0,"serving":150,"category":"korean"},
    "닭똥집구이": {"cal":94,"prot":17.7,"carbs":0,"fat":2.6,"fiber":0,"sodium":68,"sugar":0,"serving":80,"category":"korean"},
    "곱창전골": {"cal":70,"prot":6.5,"carbs":4.0,"fat":3.0,"fiber":0.5,"sodium":380,"sugar":1.0,"serving":400,"category":"korean"},
    "낙곱새": {"cal":80,"prot":8.0,"carbs":5.0,"fat":3.0,"fiber":0.5,"sodium":400,"sugar":1.5,"serving":400,"category":"korean"},

    # 밥류/면류
    "비빔밥(저칼로리)": {"cal":110,"prot":5.0,"carbs":18.0,"fat":2.0,"fiber":2.5,"sodium":350,"sugar":2.0,"serving":300,"category":"korean"},
    "닭죽": {"cal":55,"prot":4.0,"carbs":8.0,"fat":1.0,"fiber":0.2,"sodium":300,"sugar":0.5,"serving":300,"category":"korean"},
    "야채죽": {"cal":40,"prot":1.5,"carbs":8.0,"fat":0.5,"fiber":1.0,"sodium":280,"sugar":1.0,"serving":300,"category":"korean"},
    "전복죽": {"cal":50,"prot":3.5,"carbs":8.5,"fat":0.5,"fiber":0.2,"sodium":300,"sugar":0.3,"serving":300,"category":"korean"},
    "김치볶음밥(저지방)": {"cal":120,"prot":4.0,"carbs":22.0,"fat":2.0,"fiber":1.5,"sodium":400,"sugar":1.5,"serving":250,"category":"korean"},
    "나물비빔밥": {"cal":100,"prot":4.5,"carbs":17.0,"fat":1.5,"fiber":3.0,"sodium":320,"sugar":1.5,"serving":300,"category":"korean"},
    "곤드레밥": {"cal":130,"prot":3.5,"carbs":26.0,"fat":1.5,"fiber":2.0,"sodium":250,"sugar":0.5,"serving":250,"category":"korean"},
    "콩나물밥": {"cal":120,"prot":4.0,"carbs":23.0,"fat":1.0,"fiber":1.5,"sodium":200,"sugar":0.5,"serving":250,"category":"korean"},
    "쌈밥": {"cal":110,"prot":5.0,"carbs":18.0,"fat":2.0,"fiber":2.0,"sodium":280,"sugar":1.0,"serving":300,"category":"korean"},
    "냉메밀국수": {"cal":115,"prot":5.0,"carbs":24.0,"fat":0.5,"fiber":1.5,"sodium":400,"sugar":2.0,"serving":300,"category":"korean"},

    # ── 1000종 확장: snack_drink 건강 옵션 ──
    "프로틴볼": {"cal":380,"prot":20.0,"carbs":35.0,"fat":18.0,"fiber":4.0,"sodium":120,"sugar":12.0,"serving":30,"category":"snack_drink"},
    "에너지볼": {"cal":400,"prot":10.0,"carbs":45.0,"fat":22.0,"fiber":5.0,"sodium":50,"sugar":18.0,"serving":25,"category":"snack_drink"},
    "건과일믹스": {"cal":320,"prot":3.0,"carbs":78.0,"fat":1.0,"fiber":5.0,"sodium":15,"sugar":55.0,"serving":30,"category":"snack_drink"},
    "견과믹스": {"cal":607,"prot":17.0,"carbs":21.0,"fat":54.0,"fiber":8.0,"sodium":5,"sugar":5.0,"serving":25,"category":"snack_drink"},
    "현미과자": {"cal":410,"prot":6.0,"carbs":73.0,"fat":10.0,"fiber":2.5,"sodium":280,"sugar":8.0,"serving":25,"category":"snack_drink"},
    "뻥튀기": {"cal":380,"prot":7.0,"carbs":82.0,"fat":2.5,"fiber":2.0,"sodium":200,"sugar":3.0,"serving":20,"category":"snack_drink"},
    "곤약젤리(음료)": {"cal":5,"prot":0,"carbs":1.2,"fat":0,"fiber":0.8,"sodium":20,"sugar":0,"serving":150,"category":"snack_drink"},
    "무설탕캔디": {"cal":240,"prot":0,"carbs":97.0,"fat":0,"fiber":0,"sodium":10,"sugar":0,"serving":10,"category":"snack_drink"},
    "다크초콜릿85": {"cal":550,"prot":9.0,"carbs":30.0,"fat":46.0,"fiber":12.0,"sodium":10,"sugar":14.0,"serving":20,"category":"snack_drink"},
    "카카오닙스": {"cal":500,"prot":12.0,"carbs":35.0,"fat":40.0,"fiber":25.0,"sodium":10,"sugar":2.0,"serving":15,"category":"snack_drink"},
    "라이스케이크": {"cal":387,"prot":8.0,"carbs":82.0,"fat":2.8,"fiber":1.7,"sodium":230,"sugar":0.5,"serving":10,"category":"snack_drink"},
    "콤부차": {"cal":15,"prot":0,"carbs":3.5,"fat":0,"fiber":0,"sodium":5,"sugar":2.5,"serving":250,"category":"snack_drink"},
    "아사이주스": {"cal":45,"prot":0.5,"carbs":10.0,"fat":0.5,"fiber":1.0,"sodium":5,"sugar":7.0,"serving":200,"category":"snack_drink"},
    "당근주스": {"cal":40,"prot":0.9,"carbs":9.3,"fat":0.2,"fiber":0.8,"sodium":30,"sugar":4.0,"serving":200,"category":"snack_drink"},
    "토마토주스": {"cal":17,"prot":0.8,"carbs":3.5,"fat":0.1,"fiber":0.4,"sodium":200,"sugar":2.6,"serving":200,"category":"snack_drink"},
    "양배추주스": {"cal":22,"prot":1.0,"carbs":4.5,"fat":0.1,"fiber":1.0,"sodium":15,"sugar":3.0,"serving":200,"category":"snack_drink"},
    "셀러리주스": {"cal":16,"prot":0.7,"carbs":3.0,"fat":0.2,"fiber":1.6,"sodium":80,"sugar":1.3,"serving":200,"category":"snack_drink"},
    "비트주스": {"cal":43,"prot":1.6,"carbs":10.0,"fat":0.2,"fiber":0.8,"sodium":78,"sugar":7.0,"serving":200,"category":"snack_drink"},
    "디톡스워터": {"cal":5,"prot":0,"carbs":1.0,"fat":0,"fiber":0,"sodium":2,"sugar":0.5,"serving":500,"category":"snack_drink"},
    "제로탄산음료": {"cal":0,"prot":0,"carbs":0,"fat":0,"fiber":0,"sodium":15,"sugar":0,"serving":350,"category":"snack_drink"},
    "무가당두유": {"cal":35,"prot":3.6,"carbs":1.0,"fat":1.9,"fiber":0.5,"sodium":30,"sugar":0,"serving":200,"category":"snack_drink"},
    "코코넛워터": {"cal":19,"prot":0.7,"carbs":3.7,"fat":0.2,"fiber":1.1,"sodium":105,"sugar":2.6,"serving":250,"category":"snack_drink"},
    "보리차(무가당)": {"cal":1,"prot":0,"carbs":0.2,"fat":0,"fiber":0,"sodium":2,"sugar":0,"serving":500,"category":"snack_drink"},
    "옥수수수염차": {"cal":0,"prot":0,"carbs":0,"fat":0,"fiber":0,"sodium":3,"sugar":0,"serving":500,"category":"snack_drink"},
    "호박라떼": {"cal":55,"prot":1.5,"carbs":10.0,"fat":1.5,"fiber":0.5,"sodium":40,"sugar":7.0,"serving":300,"category":"snack_drink"},

    # ── 1000종 확장: franchise 보강 ──
    # 치킨 브랜드
    "굽네치킨볼케이노": {"cal":200,"prot":18.0,"carbs":8.0,"fat":10.5,"fiber":0.5,"sodium":550,"sugar":3.0,"serving":150,"category":"franchise"},
    "굽네치킨오리지널": {"cal":165,"prot":20.0,"carbs":3.0,"fat":8.0,"fiber":0,"sodium":480,"sugar":1.0,"serving":150,"category":"franchise"},
    "네네치킨스노윙": {"cal":260,"prot":16.0,"carbs":15.0,"fat":16.0,"fiber":0.5,"sodium":550,"sugar":5.0,"serving":150,"category":"franchise"},
    "페리카나양념치킨": {"cal":250,"prot":16.0,"carbs":14.0,"fat":15.0,"fiber":0.5,"sodium":580,"sugar":6.0,"serving":150,"category":"franchise"},
    "호식이두마리치킨": {"cal":255,"prot":17.0,"carbs":12.0,"fat":16.0,"fiber":0.5,"sodium":520,"sugar":4.0,"serving":150,"category":"franchise"},

    # 배달 인기 메뉴
    "족발(앞다리)": {"cal":200,"prot":22.0,"carbs":1.0,"fat":12.0,"fiber":0,"sodium":380,"sugar":0.5,"serving":150,"category":"franchise"},
    "보쌈(삶은)": {"cal":215,"prot":21.0,"carbs":1.0,"fat":14.0,"fiber":0,"sodium":350,"sugar":0.5,"serving":150,"category":"franchise"},
    "찜닭": {"cal":120,"prot":12.0,"carbs":8.0,"fat":4.5,"fiber":0.5,"sodium":420,"sugar":4.0,"serving":200,"category":"franchise"},
    "엽기떡볶이로제": {"cal":165,"prot":5.0,"carbs":22.0,"fat":6.5,"fiber":1.0,"sodium":500,"sugar":5.0,"serving":250,"category":"franchise"},
    "떡볶이(밀떡)": {"cal":180,"prot":4.0,"carbs":30.0,"fat":5.0,"fiber":1.0,"sodium":550,"sugar":6.0,"serving":250,"category":"franchise"},
    "치킨무": {"cal":30,"prot":0.3,"carbs":7.0,"fat":0,"fiber":0.5,"sodium":350,"sugar":5.0,"serving":50,"category":"franchise"},
    "양념감자": {"cal":170,"prot":2.0,"carbs":22.0,"fat":8.0,"fiber":2.0,"sodium":350,"sugar":1.5,"serving":100,"category":"franchise"},
    "치즈볼": {"cal":300,"prot":8.0,"carbs":30.0,"fat":17.0,"fiber":0.5,"sodium":420,"sugar":3.0,"serving":40,"category":"franchise"},

    # 분식
    "떡볶이(쌀떡)": {"cal":150,"prot":3.5,"carbs":28.0,"fat":3.0,"fiber":0.5,"sodium":480,"sugar":5.0,"serving":250,"category":"franchise"},
    "순대(당면)": {"cal":180,"prot":9.0,"carbs":25.0,"fat":5.0,"fiber":0.5,"sodium":400,"sugar":1.0,"serving":150,"category":"franchise"},
    "모듬튀김": {"cal":250,"prot":6.0,"carbs":25.0,"fat":14.0,"fiber":1.5,"sodium":350,"sugar":2.0,"serving":120,"category":"franchise"},
    "오징어튀김": {"cal":220,"prot":10.0,"carbs":20.0,"fat":11.0,"fiber":0.5,"sodium":400,"sugar":1.0,"serving":80,"category":"franchise"},
    "김말이튀김": {"cal":230,"prot":5.0,"carbs":28.0,"fat":11.0,"fiber":1.0,"sodium":380,"sugar":1.5,"serving":80,"category":"franchise"},
    "고구마튀김": {"cal":200,"prot":2.0,"carbs":30.0,"fat":8.0,"fiber":2.0,"sodium":200,"sugar":5.0,"serving":60,"category":"franchise"},
    "야끼만두": {"cal":200,"prot":7.0,"carbs":22.0,"fat":9.0,"fiber":1.0,"sodium":420,"sugar":2.0,"serving":100,"category":"franchise"},

    # 편의점 다이어트 옵션
    "편의점곤약면": {"cal":15,"prot":0.5,"carbs":3.0,"fat":0,"fiber":2.0,"sodium":300,"sugar":0.5,"serving":200,"category":"franchise"},
    "편의점프로틴바": {"cal":380,"prot":30.0,"carbs":32.0,"fat":15.0,"fiber":3.0,"sodium":200,"sugar":5.0,"serving":50,"category":"franchise"},
    "편의점두부바": {"cal":90,"prot":8.0,"carbs":6.0,"fat":4.0,"fiber":1.0,"sodium":200,"sugar":2.0,"serving":100,"category":"franchise"},
    "편의점그릭요거트": {"cal":65,"prot":9.0,"carbs":5.0,"fat":0.8,"fiber":0,"sodium":50,"sugar":4.5,"serving":130,"category":"franchise"},
    "편의점삶은달걀": {"cal":155,"prot":12.6,"carbs":1.1,"fat":11.0,"fiber":0,"sodium":300,"sugar":0.5,"serving":55,"category":"franchise"},
    "편의점닭가슴살바": {"cal":110,"prot":20.0,"carbs":3.0,"fat":2.0,"fiber":0,"sodium":450,"sugar":1.0,"serving":100,"category":"franchise"},
    "편의점프로틴쉐이크": {"cal":60,"prot":10.0,"carbs":4.0,"fat":0.5,"fiber":0,"sodium":80,"sugar":2.0,"serving":250,"category":"franchise"},
    "편의점에너지바": {"cal":420,"prot":8.0,"carbs":55.0,"fat":20.0,"fiber":3.0,"sodium":150,"sugar":20.0,"serving":40,"category":"franchise"},

    # 스타벅스/카페 추가
    "아이스아메리카노": {"cal":10,"prot":0.5,"carbs":2.0,"fat":0,"fiber":0,"sodium":5,"sugar":0,"serving":473,"category":"franchise"},
    "연유라떼": {"cal":55,"prot":2.0,"carbs":9.0,"fat":1.5,"fiber":0,"sodium":45,"sugar":8.0,"serving":473,"category":"franchise"},
    "바닐라빈라떼": {"cal":45,"prot":2.5,"carbs":7.0,"fat":1.0,"fiber":0,"sodium":55,"sugar":6.5,"serving":473,"category":"franchise"},
    "콜드브루": {"cal":5,"prot":0.3,"carbs":1.0,"fat":0,"fiber":0,"sodium":3,"sugar":0,"serving":473,"category":"franchise"},
    "플랫화이트": {"cal":35,"prot":2.5,"carbs":3.5,"fat":1.5,"fiber":0,"sodium":40,"sugar":3.5,"serving":355,"category":"franchise"},
    "디카페인아메리카노": {"cal":10,"prot":0.5,"carbs":2.0,"fat":0,"fiber":0,"sodium":5,"sugar":0,"serving":473,"category":"franchise"},

    # ── 1000종 확장: foreign_popular 보강 ──
    # 건강한 외국 음식
    "포케볼": {"cal":120,"prot":10.0,"carbs":15.0,"fat":3.0,"fiber":1.5,"sodium":350,"sugar":3.0,"serving":300,"category":"foreign_popular"},
    "아히포케": {"cal":130,"prot":12.0,"carbs":14.0,"fat":4.0,"fiber":1.0,"sodium":400,"sugar":3.0,"serving":250,"category":"foreign_popular"},
    "훔무스": {"cal":166,"prot":7.9,"carbs":14.3,"fat":9.6,"fiber":6.0,"sodium":379,"sugar":0.3,"serving":60,"category":"foreign_popular"},
    "바바가누시": {"cal":115,"prot":2.5,"carbs":10.0,"fat":7.5,"fiber":3.0,"sodium":300,"sugar":3.0,"serving":60,"category":"foreign_popular"},
    "가스파초": {"cal":25,"prot":0.7,"carbs":4.5,"fat":0.5,"fiber":0.8,"sodium":250,"sugar":3.0,"serving":300,"category":"foreign_popular"},
    "미네스트로네수프": {"cal":35,"prot":1.5,"carbs":6.0,"fat":0.7,"fiber":1.5,"sodium":350,"sugar":2.0,"serving":300,"category":"foreign_popular"},
    "치킨샐러드": {"cal":110,"prot":12.0,"carbs":5.0,"fat":5.0,"fiber":1.5,"sodium":300,"sugar":2.0,"serving":200,"category":"foreign_popular"},
    "참치샐러드": {"cal":95,"prot":14.0,"carbs":3.0,"fat":3.5,"fiber":0.5,"sodium":350,"sugar":1.0,"serving":150,"category":"foreign_popular"},
    "퀴노아볼": {"cal":180,"prot":6.0,"carbs":28.0,"fat":5.0,"fiber":3.5,"sodium":250,"sugar":2.0,"serving":150,"category":"foreign_popular"},
    "라타투이": {"cal":40,"prot":1.0,"carbs":6.0,"fat":1.5,"fiber":2.0,"sodium":200,"sugar":3.5,"serving":200,"category":"foreign_popular"},
    "무사카": {"cal":130,"prot":7.0,"carbs":10.0,"fat":7.0,"fiber":2.0,"sodium":350,"sugar":3.0,"serving":200,"category":"foreign_popular"},
    "타불레": {"cal":120,"prot":3.0,"carbs":16.0,"fat":5.0,"fiber":2.5,"sodium":250,"sugar":1.5,"serving":150,"category":"foreign_popular"},
    "팔라펠": {"cal":333,"prot":13.3,"carbs":31.8,"fat":17.8,"fiber":0,"sodium":294,"sugar":0,"serving":50,"category":"foreign_popular"},
    "샤와르마": {"cal":175,"prot":15.0,"carbs":8.0,"fat":9.0,"fiber":1.0,"sodium":400,"sugar":2.0,"serving":150,"category":"foreign_popular"},
    "케밥": {"cal":150,"prot":14.0,"carbs":5.0,"fat":8.0,"fiber":1.0,"sodium":380,"sugar":2.0,"serving":150,"category":"foreign_popular"},
    "지중해샐러드": {"cal":85,"prot":3.0,"carbs":6.0,"fat":6.0,"fiber":2.0,"sodium":300,"sugar":3.0,"serving":200,"category":"foreign_popular"},
    "감바스": {"cal":160,"prot":12.0,"carbs":2.0,"fat":12.0,"fiber":0.3,"sodium":350,"sugar":0.5,"serving":120,"category":"foreign_popular"},
    "칠리콘카르네": {"cal":85,"prot":6.0,"carbs":8.0,"fat":3.0,"fiber":3.0,"sodium":400,"sugar":2.5,"serving":200,"category":"foreign_popular"},
    "볼로네제소스": {"cal":95,"prot":6.0,"carbs":5.0,"fat":5.5,"fiber":1.0,"sodium":380,"sugar":3.0,"serving":150,"category":"foreign_popular"},
    "마르게리타피자": {"cal":250,"prot":11.0,"carbs":30.0,"fat":10.0,"fiber":1.5,"sodium":480,"sugar":3.5,"serving":120,"category":"foreign_popular"},
    "카프리치오사": {"cal":220,"prot":10.0,"carbs":28.0,"fat":8.0,"fiber":1.5,"sodium":450,"sugar":3.0,"serving":120,"category":"foreign_popular"},
    "반미": {"cal":210,"prot":10.0,"carbs":28.0,"fat":7.0,"fiber":2.0,"sodium":500,"sugar":4.0,"serving":180,"category":"foreign_popular"},
    "쌀국수": {"cal":50,"prot":2.5,"carbs":9.0,"fat":0.5,"fiber":0.3,"sodium":350,"sugar":1.0,"serving":400,"category":"foreign_popular"},
    "팟타이": {"cal":140,"prot":6.0,"carbs":20.0,"fat":4.5,"fiber":1.0,"sodium":450,"sugar":5.0,"serving":250,"category":"foreign_popular"},
    "카오팟": {"cal":155,"prot":5.5,"carbs":22.0,"fat":5.0,"fiber":1.0,"sodium":400,"sugar":2.0,"serving":250,"category":"foreign_popular"},
    "솜땀": {"cal":50,"prot":1.5,"carbs":10.0,"fat":0.5,"fiber":2.0,"sodium":380,"sugar":6.0,"serving":150,"category":"foreign_popular"},
    "나시고렝": {"cal":160,"prot":6.0,"carbs":23.0,"fat":5.0,"fiber":1.0,"sodium":420,"sugar":2.5,"serving":250,"category":"foreign_popular"},
    "볶음쌀국수": {"cal":130,"prot":5.0,"carbs":18.0,"fat":4.0,"fiber":0.8,"sodium":380,"sugar":2.0,"serving":250,"category":"foreign_popular"},
    "분짜": {"cal":90,"prot":6.5,"carbs":12.0,"fat":2.0,"fiber":0.5,"sodium":400,"sugar":4.0,"serving":300,"category":"foreign_popular"},

    # ── 1000종 확장: vegetable 보강 ──
    "청경채": {"cal":13,"prot":1.5,"carbs":2.2,"fat":0.2,"fiber":1.0,"sodium":65,"sugar":1.2,"serving":100,"category":"vegetable"},
    "로메인": {"cal":17,"prot":1.2,"carbs":3.3,"fat":0.3,"fiber":2.1,"sodium":8,"sugar":1.2,"serving":80,"category":"vegetable"},
    "치커리": {"cal":23,"prot":1.7,"carbs":4.7,"fat":0.3,"fiber":3.1,"sodium":2,"sugar":0.7,"serving":50,"category":"vegetable"},
    "콜라비": {"cal":27,"prot":1.7,"carbs":6.2,"fat":0.1,"fiber":3.6,"sodium":20,"sugar":2.6,"serving":100,"category":"vegetable"},
    "래디시": {"cal":16,"prot":0.7,"carbs":3.4,"fat":0.1,"fiber":1.6,"sodium":39,"sugar":1.9,"serving":50,"category":"vegetable"},
    "순무": {"cal":28,"prot":0.9,"carbs":6.4,"fat":0.1,"fiber":1.8,"sodium":67,"sugar":3.8,"serving":100,"category":"vegetable"},
    "고구마줄기": {"cal":18,"prot":1.0,"carbs":3.5,"fat":0.1,"fiber":2.0,"sodium":5,"sugar":0.5,"serving":60,"category":"vegetable"},
    "노각": {"cal":12,"prot":0.5,"carbs":2.8,"fat":0.1,"fiber":0.8,"sodium":3,"sugar":1.5,"serving":80,"category":"vegetable"},
    "브로콜리새싹": {"cal":35,"prot":2.8,"carbs":5.6,"fat":0.6,"fiber":2.0,"sodium":6,"sugar":0.5,"serving":30,"category":"vegetable"},
    "알팔파": {"cal":23,"prot":4.0,"carbs":2.1,"fat":0.7,"fiber":1.9,"sodium":6,"sugar":0.2,"serving":20,"category":"vegetable"},
    "비타민": {"cal":13,"prot":1.5,"carbs":2.0,"fat":0.2,"fiber":1.2,"sodium":50,"sugar":0.8,"serving":80,"category":"vegetable"},
    "쑥갓": {"cal":24,"prot":3.4,"carbs":3.0,"fat":0.6,"fiber":3.0,"sodium":53,"sugar":0.5,"serving":50,"category":"vegetable"},
    "근대": {"cal":19,"prot":1.8,"carbs":3.7,"fat":0.2,"fiber":1.6,"sodium":213,"sugar":1.1,"serving":60,"category":"vegetable"},
    "열무": {"cal":17,"prot":1.8,"carbs":2.6,"fat":0.3,"fiber":2.1,"sodium":20,"sugar":0.8,"serving":70,"category":"vegetable"},
    "얼갈이배추": {"cal":12,"prot":1.2,"carbs":2.0,"fat":0.1,"fiber":1.2,"sodium":18,"sugar":1.0,"serving":100,"category":"vegetable"},

    # ── 1000종 달성: diet_fitness 추가 2차 ──
    # 밀프렙/도시락 재료
    "닭가슴살볶음밥": {"cal":130,"prot":12.0,"carbs":16.0,"fat":2.5,"fiber":0.5,"sodium":350,"sugar":1.0,"serving":250,"category":"diet_fitness"},
    "닭가슴살카레": {"cal":85,"prot":10.0,"carbs":8.0,"fat":2.0,"fiber":1.0,"sodium":300,"sugar":2.0,"serving":250,"category":"diet_fitness"},
    "닭가슴살떡볶이": {"cal":100,"prot":10.0,"carbs":12.0,"fat":2.0,"fiber":0.5,"sodium":350,"sugar":3.0,"serving":250,"category":"diet_fitness"},
    "닭가슴살김밥": {"cal":140,"prot":10.0,"carbs":20.0,"fat":3.0,"fiber":1.0,"sodium":350,"sugar":2.0,"serving":250,"category":"diet_fitness"},
    "닭가슴살파스타": {"cal":115,"prot":11.0,"carbs":14.0,"fat":2.0,"fiber":1.0,"sodium":320,"sugar":1.5,"serving":250,"category":"diet_fitness"},
    "닭가슴살덮밥": {"cal":120,"prot":11.0,"carbs":16.0,"fat":2.0,"fiber":0.5,"sodium":330,"sugar":1.5,"serving":250,"category":"diet_fitness"},
    "곤약볶음면": {"cal":25,"prot":1.0,"carbs":5.0,"fat":0.3,"fiber":3.0,"sodium":250,"sugar":1.0,"serving":200,"category":"diet_fitness"},
    "두부김치": {"cal":80,"prot":6.5,"carbs":5.0,"fat":4.0,"fiber":1.5,"sodium":350,"sugar":1.5,"serving":200,"category":"diet_fitness"},
    "두부샐러드": {"cal":55,"prot":5.0,"carbs":4.0,"fat":2.5,"fiber":1.5,"sodium":180,"sugar":2.0,"serving":200,"category":"diet_fitness"},
    "연어샐러드": {"cal":95,"prot":10.0,"carbs":5.0,"fat":4.0,"fiber":1.5,"sodium":250,"sugar":2.0,"serving":200,"category":"diet_fitness"},
    "참치샐러드볼": {"cal":90,"prot":12.0,"carbs":5.0,"fat":3.0,"fiber":1.5,"sodium":280,"sugar":1.5,"serving":200,"category":"diet_fitness"},
    "새우샐러드": {"cal":70,"prot":10.0,"carbs":4.0,"fat":1.5,"fiber":1.5,"sodium":250,"sugar":2.0,"serving":200,"category":"diet_fitness"},
    "그릭요거트볼": {"cal":80,"prot":8.0,"carbs":8.0,"fat":2.0,"fiber":1.5,"sodium":40,"sugar":5.0,"serving":200,"category":"diet_fitness"},
    "오버나이트오트": {"cal":120,"prot":5.0,"carbs":18.0,"fat":3.0,"fiber":3.0,"sodium":30,"sugar":5.0,"serving":200,"category":"diet_fitness"},
    "프로틴볼(수제)": {"cal":350,"prot":18.0,"carbs":30.0,"fat":18.0,"fiber":5.0,"sodium":80,"sugar":10.0,"serving":30,"category":"diet_fitness"},
    "고구마샐러드": {"cal":90,"prot":1.5,"carbs":18.0,"fat":1.5,"fiber":2.5,"sodium":150,"sugar":5.0,"serving":150,"category":"diet_fitness"},
    "닭가슴살주먹밥": {"cal":145,"prot":10.0,"carbs":22.0,"fat":2.5,"fiber":0.5,"sodium":300,"sugar":1.0,"serving":120,"category":"diet_fitness"},
    "현미주먹밥": {"cal":160,"prot":3.5,"carbs":34.0,"fat":1.0,"fiber":1.5,"sodium":200,"sugar":0.5,"serving":120,"category":"diet_fitness"},
    "곤약떡볶이": {"cal":30,"prot":1.0,"carbs":6.0,"fat":0.2,"fiber":2.5,"sodium":300,"sugar":2.0,"serving":250,"category":"diet_fitness"},

    # 스무디/건강음료
    "그린스무디": {"cal":45,"prot":1.5,"carbs":8.0,"fat":1.0,"fiber":2.0,"sodium":20,"sugar":5.0,"serving":300,"category":"diet_fitness"},
    "프로틴스무디": {"cal":70,"prot":10.0,"carbs":8.0,"fat":1.0,"fiber":1.0,"sodium":50,"sugar":5.0,"serving":300,"category":"diet_fitness"},
    "바나나프로틴쉐이크": {"cal":80,"prot":10.0,"carbs":10.0,"fat":1.0,"fiber":1.0,"sodium":60,"sugar":6.0,"serving":300,"category":"diet_fitness"},
    "베리스무디": {"cal":50,"prot":1.0,"carbs":11.0,"fat":0.5,"fiber":2.0,"sodium":5,"sugar":7.0,"serving":300,"category":"diet_fitness"},
    "아사이스무디": {"cal":55,"prot":1.0,"carbs":12.0,"fat":0.5,"fiber":2.0,"sodium":5,"sugar":8.0,"serving":300,"category":"diet_fitness"},
    "프로틴라떼": {"cal":40,"prot":6.0,"carbs":3.0,"fat":0.5,"fiber":0,"sodium":50,"sugar":1.5,"serving":350,"category":"diet_fitness"},

    # ── 1000종 달성: korean 추가 ──
    # 김치/장아찌/젓갈
    "열무김치": {"cal":18,"prot":1.5,"carbs":2.8,"fat":0.3,"fiber":1.5,"sodium":500,"sugar":1.0,"serving":50,"category":"korean"},
    "갓김치": {"cal":20,"prot":1.8,"carbs":3.0,"fat":0.3,"fiber":1.8,"sodium":520,"sugar":1.0,"serving":50,"category":"korean"},
    "동치미": {"cal":8,"prot":0.3,"carbs":1.5,"fat":0.1,"fiber":0.3,"sodium":400,"sugar":0.5,"serving":100,"category":"korean"},
    "백김치": {"cal":15,"prot":1.2,"carbs":2.5,"fat":0.2,"fiber":1.0,"sodium":380,"sugar":1.0,"serving":50,"category":"korean"},
    "부추김치": {"cal":22,"prot":2.0,"carbs":3.0,"fat":0.5,"fiber":1.5,"sodium":480,"sugar":1.0,"serving":50,"category":"korean"},
    "오이지": {"cal":12,"prot":0.5,"carbs":2.5,"fat":0.1,"fiber":0.8,"sodium":450,"sugar":1.0,"serving":30,"category":"korean"},
    "장아찌(모듬)": {"cal":60,"prot":1.5,"carbs":13.0,"fat":0.2,"fiber":1.0,"sodium":550,"sugar":5.0,"serving":30,"category":"korean"},
    "새우젓": {"cal":55,"prot":10.0,"carbs":2.0,"fat":0.5,"fiber":0,"sodium":5500,"sugar":0.5,"serving":10,"category":"korean"},
    "명란젓": {"cal":120,"prot":20.0,"carbs":3.0,"fat":3.0,"fiber":0,"sodium":2000,"sugar":0.5,"serving":20,"category":"korean"},
    "창란젓": {"cal":75,"prot":11.0,"carbs":5.0,"fat":1.0,"fiber":0,"sodium":3000,"sugar":1.0,"serving":15,"category":"korean"},

    # 전/부침
    "부추전": {"cal":185,"prot":5.5,"carbs":20.0,"fat":9.0,"fiber":1.5,"sodium":380,"sugar":1.5,"serving":100,"category":"korean"},
    "감자전": {"cal":165,"prot":3.0,"carbs":22.0,"fat":7.5,"fiber":1.5,"sodium":300,"sugar":1.0,"serving":100,"category":"korean"},
    "녹두전": {"cal":175,"prot":8.0,"carbs":18.0,"fat":8.0,"fiber":2.0,"sodium":350,"sugar":1.0,"serving":100,"category":"korean"},
    "호박전": {"cal":150,"prot":4.0,"carbs":15.0,"fat":8.0,"fiber":1.0,"sodium":280,"sugar":2.0,"serving":80,"category":"korean"},
    "굴전": {"cal":170,"prot":8.0,"carbs":16.0,"fat":8.0,"fiber":0.5,"sodium":350,"sugar":1.0,"serving":80,"category":"korean"},
    "동태전": {"cal":160,"prot":12.0,"carbs":12.0,"fat":7.0,"fiber":0.3,"sodium":320,"sugar":0.5,"serving":80,"category":"korean"},

    # 찜/조림
    "아귀찜": {"cal":80,"prot":10.0,"carbs":5.0,"fat":2.5,"fiber":1.0,"sodium":400,"sugar":2.0,"serving":200,"category":"korean"},
    "해물찜": {"cal":75,"prot":9.0,"carbs":5.0,"fat":2.0,"fiber":0.5,"sodium":420,"sugar":2.0,"serving":200,"category":"korean"},
    "코다리찜": {"cal":85,"prot":12.0,"carbs":5.0,"fat":2.0,"fiber":0.5,"sodium":400,"sugar":2.5,"serving":200,"category":"korean"},
    "닭도리탕": {"cal":95,"prot":10.0,"carbs":6.0,"fat":3.5,"fiber":1.0,"sodium":380,"sugar":2.5,"serving":250,"category":"korean"},
    "조기조림": {"cal":110,"prot":14.0,"carbs":5.0,"fat":3.5,"fiber":0.3,"sodium":400,"sugar":2.5,"serving":100,"category":"korean"},
    "고등어조림": {"cal":150,"prot":15.0,"carbs":5.0,"fat":7.5,"fiber":0.5,"sodium":420,"sugar":3.0,"serving":120,"category":"korean"},
    "갈치조림": {"cal":130,"prot":13.0,"carbs":6.0,"fat":5.5,"fiber":0.5,"sodium":430,"sugar":3.0,"serving":120,"category":"korean"},
    "두부전골": {"cal":40,"prot":3.5,"carbs":3.0,"fat":1.5,"fiber":0.5,"sodium":350,"sugar":1.0,"serving":400,"category":"korean"},
    "버섯전골": {"cal":25,"prot":2.0,"carbs":3.0,"fat":0.5,"fiber":1.0,"sodium":300,"sugar":1.0,"serving":400,"category":"korean"},

    # 밥/면 추가
    "보리밥": {"cal":148,"prot":3.5,"carbs":32.0,"fat":0.5,"fiber":2.5,"sodium":3,"sugar":0.3,"serving":210,"category":"korean"},
    "팥밥": {"cal":155,"prot":5.0,"carbs":33.0,"fat":0.4,"fiber":2.0,"sodium":5,"sugar":0.5,"serving":210,"category":"korean"},
    "기장밥": {"cal":150,"prot":3.5,"carbs":33.0,"fat":0.5,"fiber":1.5,"sodium":3,"sugar":0.3,"serving":210,"category":"korean"},
    "수제비": {"cal":85,"prot":3.0,"carbs":16.0,"fat":1.0,"fiber":0.5,"sodium":350,"sugar":0.5,"serving":350,"category":"korean"},
    "들깨칼국수": {"cal":100,"prot":4.0,"carbs":15.0,"fat":3.0,"fiber":1.0,"sodium":380,"sugar":0.5,"serving":350,"category":"korean"},

    # ── 1000종 달성: snack_drink 추가 ──
    "프로틴그래놀라": {"cal":400,"prot":15.0,"carbs":52.0,"fat":15.0,"fiber":6.0,"sodium":100,"sugar":12.0,"serving":40,"category":"snack_drink"},
    "무설탕그래놀라": {"cal":420,"prot":10.0,"carbs":62.0,"fat":16.0,"fiber":7.0,"sodium":50,"sugar":2.0,"serving":40,"category":"snack_drink"},
    "프로틴머핀": {"cal":280,"prot":18.0,"carbs":25.0,"fat":12.0,"fiber":3.0,"sodium":250,"sugar":6.0,"serving":60,"category":"snack_drink"},
    "프로틴와플": {"cal":200,"prot":15.0,"carbs":20.0,"fat":7.0,"fiber":2.0,"sodium":280,"sugar":3.0,"serving":80,"category":"snack_drink"},
    "현미떡": {"cal":215,"prot":4.0,"carbs":48.0,"fat":0.5,"fiber":1.5,"sodium":180,"sugar":3.0,"serving":50,"category":"snack_drink"},
    "흑미떡": {"cal":220,"prot":4.5,"carbs":47.0,"fat":1.0,"fiber":2.0,"sodium":170,"sugar":3.0,"serving":50,"category":"snack_drink"},
    "인절미": {"cal":230,"prot":4.0,"carbs":50.0,"fat":1.5,"fiber":0.5,"sodium":150,"sugar":5.0,"serving":50,"category":"snack_drink"},
    "약과": {"cal":400,"prot":5.0,"carbs":55.0,"fat":18.0,"fiber":1.0,"sodium":100,"sugar":20.0,"serving":35,"category":"snack_drink"},
    "백설기": {"cal":220,"prot":3.5,"carbs":50.0,"fat":0.5,"fiber":0.3,"sodium":100,"sugar":8.0,"serving":60,"category":"snack_drink"},
    "카스테라": {"cal":320,"prot":7.0,"carbs":52.0,"fat":10.0,"fiber":0.5,"sodium":200,"sugar":25.0,"serving":50,"category":"snack_drink"},
    "쑥떡": {"cal":210,"prot":4.0,"carbs":46.0,"fat":1.0,"fiber":2.0,"sodium":150,"sugar":8.0,"serving":50,"category":"snack_drink"},
    "두유(바나나)": {"cal":55,"prot":2.5,"carbs":9.0,"fat":1.5,"fiber":0.3,"sodium":80,"sugar":7.0,"serving":200,"category":"snack_drink"},
    "두유(검은콩)": {"cal":45,"prot":3.5,"carbs":5.5,"fat":1.5,"fiber":0.5,"sodium":70,"sugar":4.0,"serving":200,"category":"snack_drink"},
    "식혜": {"cal":55,"prot":0.3,"carbs":13.0,"fat":0,"fiber":0.2,"sodium":15,"sugar":10.0,"serving":200,"category":"snack_drink"},
    "수정과": {"cal":40,"prot":0.1,"carbs":10.0,"fat":0,"fiber":0.2,"sodium":5,"sugar":8.0,"serving":200,"category":"snack_drink"},
    "미숫가루": {"cal":370,"prot":10.0,"carbs":72.0,"fat":5.0,"fiber":4.0,"sodium":10,"sugar":5.0,"serving":30,"category":"snack_drink"},
    "율무차": {"cal":15,"prot":0.5,"carbs":3.0,"fat":0.2,"fiber":0.3,"sodium":3,"sugar":1.0,"serving":200,"category":"snack_drink"},
    "매실액": {"cal":150,"prot":0.2,"carbs":37.0,"fat":0,"fiber":0,"sodium":5,"sugar":35.0,"serving":15,"category":"snack_drink"},

    # ── 1000종 달성: franchise 추가 ──
    # 김밥/분식 프랜차이즈
    "참치김밥": {"cal":155,"prot":6.0,"carbs":24.0,"fat":4.0,"fiber":1.0,"sodium":400,"sugar":2.0,"serving":250,"category":"franchise"},
    "소고기김밥": {"cal":160,"prot":7.0,"carbs":23.0,"fat":4.5,"fiber":1.0,"sodium":420,"sugar":2.0,"serving":250,"category":"franchise"},
    "치즈김밥": {"cal":170,"prot":6.5,"carbs":23.0,"fat":5.5,"fiber":0.8,"sodium":430,"sugar":2.0,"serving":250,"category":"franchise"},
    "야채김밥": {"cal":130,"prot":3.5,"carbs":24.0,"fat":2.5,"fiber":1.5,"sodium":350,"sugar":2.0,"serving":250,"category":"franchise"},
    "충무김밥": {"cal":140,"prot":3.0,"carbs":28.0,"fat":1.5,"fiber":0.5,"sodium":300,"sugar":1.5,"serving":200,"category":"franchise"},
    "키토김밥": {"cal":100,"prot":6.0,"carbs":5.0,"fat":6.5,"fiber":1.5,"sodium":300,"sugar":1.0,"serving":200,"category":"franchise"},

    # 프랜차이즈 밥류
    "김치찌개백반": {"cal":100,"prot":5.0,"carbs":14.0,"fat":3.0,"fiber":1.5,"sodium":450,"sugar":2.0,"serving":400,"category":"franchise"},
    "된장찌개백반": {"cal":90,"prot":4.5,"carbs":13.0,"fat":2.5,"fiber":1.5,"sodium":420,"sugar":1.5,"serving":400,"category":"franchise"},
    "제육정식": {"cal":130,"prot":9.0,"carbs":15.0,"fat":4.5,"fiber":1.0,"sodium":400,"sugar":3.0,"serving":400,"category":"franchise"},
    "생선구이정식": {"cal":95,"prot":10.0,"carbs":11.0,"fat":2.5,"fiber":1.0,"sodium":380,"sugar":1.5,"serving":400,"category":"franchise"},
    "불고기백반": {"cal":110,"prot":8.0,"carbs":14.0,"fat":3.0,"fiber":1.0,"sodium":400,"sugar":2.5,"serving":400,"category":"franchise"},

    # 패스트푸드 추가
    "맥크리스피디럭스": {"cal":230,"prot":12.0,"carbs":20.0,"fat":12.0,"fiber":1.0,"sodium":480,"sugar":4.0,"serving":200,"category":"franchise"},
    "상하이버거": {"cal":245,"prot":13.0,"carbs":22.0,"fat":12.0,"fiber":1.0,"sodium":520,"sugar":4.0,"serving":200,"category":"franchise"},
    "치킨텐더": {"cal":240,"prot":18.0,"carbs":14.0,"fat":13.0,"fiber":0.5,"sodium":500,"sugar":1.0,"serving":120,"category":"franchise"},
    "어니언링": {"cal":280,"prot":4.0,"carbs":30.0,"fat":16.0,"fiber":1.5,"sodium":400,"sugar":3.0,"serving":80,"category":"franchise"},
    "해쉬브라운": {"cal":260,"prot":2.5,"carbs":28.0,"fat":16.0,"fiber":2.0,"sodium":380,"sugar":0.5,"serving":60,"category":"franchise"},
    "에그머핀": {"cal":210,"prot":12.0,"carbs":18.0,"fat":10.0,"fiber":1.0,"sodium":550,"sugar":2.0,"serving":130,"category":"franchise"},
    "그릴드치킨버거": {"cal":180,"prot":16.0,"carbs":18.0,"fat":5.0,"fiber":1.0,"sodium":420,"sugar":3.0,"serving":200,"category":"franchise"},
    "불고기버거": {"cal":220,"prot":10.0,"carbs":25.0,"fat":9.0,"fiber":1.0,"sodium":480,"sugar":5.0,"serving":180,"category":"franchise"},
    "새우버거": {"cal":230,"prot":9.0,"carbs":28.0,"fat":10.0,"fiber":1.0,"sodium":500,"sugar":3.0,"serving":180,"category":"franchise"},

    # ── 1000종 달성: foreign_popular 추가 ──
    "치킨카레": {"cal":110,"prot":8.0,"carbs":10.0,"fat":4.5,"fiber":1.5,"sodium":380,"sugar":3.0,"serving":250,"category":"foreign_popular"},
    "그린커리": {"cal":95,"prot":5.0,"carbs":6.0,"fat":6.0,"fiber":1.0,"sodium":400,"sugar":2.0,"serving":250,"category":"foreign_popular"},
    "레드커리": {"cal":100,"prot":5.0,"carbs":7.0,"fat":6.0,"fiber":1.0,"sodium":420,"sugar":2.5,"serving":250,"category":"foreign_popular"},
    "마살라": {"cal":120,"prot":7.0,"carbs":8.0,"fat":7.0,"fiber":2.0,"sodium":380,"sugar":2.0,"serving":200,"category":"foreign_popular"},
    "탄두리치킨": {"cal":148,"prot":22.0,"carbs":3.0,"fat":5.5,"fiber":0.5,"sodium":450,"sugar":1.5,"serving":150,"category":"foreign_popular"},
    "난": {"cal":260,"prot":8.6,"carbs":45.0,"fat":5.0,"fiber":2.0,"sodium":400,"sugar":3.5,"serving":80,"category":"foreign_popular"},
    "사모사": {"cal":260,"prot":5.0,"carbs":28.0,"fat":14.0,"fiber":2.0,"sodium":350,"sugar":2.0,"serving":60,"category":"foreign_popular"},
    "월남쌈(라이스페이퍼)": {"cal":75,"prot":3.0,"carbs":13.0,"fat":1.0,"fiber":1.0,"sodium":280,"sugar":2.0,"serving":120,"category":"foreign_popular"},
    "반쎄오": {"cal":200,"prot":6.0,"carbs":18.0,"fat":12.0,"fiber":1.0,"sodium":350,"sugar":2.0,"serving":150,"category":"foreign_popular"},
    "깐풍기": {"cal":230,"prot":15.0,"carbs":15.0,"fat":12.0,"fiber":1.0,"sodium":450,"sugar":5.0,"serving":150,"category":"foreign_popular"},
    "마라샹궈(건두부)": {"cal":120,"prot":8.0,"carbs":5.0,"fat":8.0,"fiber":2.0,"sodium":500,"sugar":1.5,"serving":200,"category":"foreign_popular"},
    "짜장면(간짜장)": {"cal":100,"prot":5.0,"carbs":12.0,"fat":4.0,"fiber":1.0,"sodium":380,"sugar":2.0,"serving":300,"category":"foreign_popular"},
    "잡채밥": {"cal":130,"prot":4.0,"carbs":22.0,"fat":3.0,"fiber":1.0,"sodium":350,"sugar":3.0,"serving":250,"category":"foreign_popular"},
    "해물볶음우동": {"cal":120,"prot":6.0,"carbs":18.0,"fat":3.0,"fiber":1.0,"sodium":400,"sugar":2.0,"serving":250,"category":"foreign_popular"},
    "가츠카레": {"cal":170,"prot":8.0,"carbs":18.0,"fat":7.0,"fiber":1.5,"sodium":400,"sugar":3.0,"serving":300,"category":"foreign_popular"},

    # ── 1000종 달성: vegetable 추가 ──
    "배추": {"cal":13,"prot":1.1,"carbs":2.4,"fat":0.2,"fiber":1.2,"sodium":65,"sugar":1.3,"serving":100,"category":"vegetable"},
    "미나리": {"cal":17,"prot":2.1,"carbs":2.5,"fat":0.3,"fiber":2.0,"sodium":45,"sugar":0.5,"serving":50,"category":"vegetable"},
    "들깻잎": {"cal":50,"prot":3.8,"carbs":6.5,"fat":1.5,"fiber":4.2,"sodium":3,"sugar":0.5,"serving":15,"category":"vegetable"},
    "참나물": {"cal":25,"prot":2.5,"carbs":4.0,"fat":0.3,"fiber":2.5,"sodium":8,"sugar":0.5,"serving":50,"category":"vegetable"},
    "냉이": {"cal":35,"prot":4.0,"carbs":5.0,"fat":0.5,"fiber":3.0,"sodium":10,"sugar":0.5,"serving":50,"category":"vegetable"},
    "달래": {"cal":45,"prot":3.5,"carbs":7.0,"fat":0.5,"fiber":3.0,"sodium":5,"sugar":1.5,"serving":30,"category":"vegetable"},
    "고춧잎": {"cal":32,"prot":3.5,"carbs":4.5,"fat":0.5,"fiber":2.5,"sodium":5,"sugar":0.5,"serving":50,"category":"vegetable"},
    "콩나물(삶은)": {"cal":33,"prot":3.8,"carbs":4.5,"fat":0.5,"fiber":1.5,"sodium":5,"sugar":1.5,"serving":80,"category":"vegetable"},
    "숙주(삶은)": {"cal":14,"prot":1.5,"carbs":2.5,"fat":0.1,"fiber":1.0,"sodium":5,"sugar":1.0,"serving":80,"category":"vegetable"},
    "도라지": {"cal":60,"prot":2.2,"carbs":13.5,"fat":0.3,"fiber":4.0,"sodium":8,"sugar":2.0,"serving":60,"category":"vegetable"},
    "고사리": {"cal":26,"prot":2.5,"carbs":4.0,"fat":0.3,"fiber":3.5,"sodium":3,"sugar":0.3,"serving":60,"category":"vegetable"},
    "취나물(삶은)": {"cal":22,"prot":2.0,"carbs":3.5,"fat":0.3,"fiber":3.0,"sodium":5,"sugar":0.3,"serving":50,"category":"vegetable"},
    "더덕": {"cal":78,"prot":1.8,"carbs":18.0,"fat":0.3,"fiber":4.0,"sodium":5,"sugar":2.0,"serving":50,"category":"vegetable"},
    "두릅": {"cal":23,"prot":3.0,"carbs":3.0,"fat":0.3,"fiber":2.5,"sodium":3,"sugar":0.5,"serving":50,"category":"vegetable"},

    # ── 최종 52종: 다이어터/운동인 핵심 보완 ──
    # diet_fitness - 운동 전후 식품
    "운동전부스터": {"cal":15,"prot":0,"carbs":3.5,"fat":0,"fiber":0,"sodium":50,"sugar":2.0,"serving":300,"category":"diet_fitness"},
    "게이너쉐이크": {"cal":400,"prot":20.0,"carbs":65.0,"fat":7.0,"fiber":1.0,"sodium":150,"sugar":15.0,"serving":100,"category":"diet_fitness"},
    "글루타민": {"cal":380,"prot":95.0,"carbs":0,"fat":0,"fiber":0,"sodium":0,"sugar":0,"serving":10,"category":"diet_fitness"},
    "크레아틴": {"cal":0,"prot":0,"carbs":0,"fat":0,"fiber":0,"sodium":0,"sugar":0,"serving":10,"category":"diet_fitness"},
    "BCAA파우더": {"cal":0,"prot":0,"carbs":0,"fat":0,"fiber":0,"sodium":0,"sugar":0,"serving":10,"category":"diet_fitness"},
    "닭가슴살칩": {"cal":350,"prot":55.0,"carbs":15.0,"fat":7.0,"fiber":0.5,"sodium":600,"sugar":3.0,"serving":30,"category":"diet_fitness"},
    "닭가슴살저키": {"cal":250,"prot":45.0,"carbs":10.0,"fat":3.0,"fiber":0,"sodium":700,"sugar":5.0,"serving":30,"category":"diet_fitness"},
    "소고기저키": {"cal":300,"prot":40.0,"carbs":20.0,"fat":6.0,"fiber":0,"sodium":650,"sugar":8.0,"serving":30,"category":"diet_fitness"},
    "바나나(운동전)": {"cal":89,"prot":1.1,"carbs":22.8,"fat":0.3,"fiber":2.6,"sodium":1,"sugar":12.2,"serving":120,"category":"diet_fitness"},
    "현미떡(운동후)": {"cal":215,"prot":4.0,"carbs":48.0,"fat":0.5,"fiber":1.5,"sodium":180,"sugar":3.0,"serving":50,"category":"diet_fitness"},
    "프로틴오트밀": {"cal":150,"prot":12.0,"carbs":20.0,"fat":3.0,"fiber":3.0,"sodium":100,"sugar":3.0,"serving":60,"category":"diet_fitness"},
    "닭가슴살꼬치": {"cal":115,"prot":22.0,"carbs":3.0,"fat":1.5,"fiber":0,"sodium":400,"sugar":1.5,"serving":100,"category":"diet_fitness"},
    "연어포케": {"cal":130,"prot":11.0,"carbs":15.0,"fat":4.0,"fiber":1.5,"sodium":350,"sugar":3.0,"serving":300,"category":"diet_fitness"},
    "프로틴아이스크림": {"cal":120,"prot":8.0,"carbs":18.0,"fat":3.0,"fiber":4.0,"sodium":80,"sugar":5.0,"serving":100,"category":"diet_fitness"},
    "닭가슴살또띠아": {"cal":130,"prot":12.0,"carbs":15.0,"fat":3.0,"fiber":1.5,"sodium":380,"sugar":2.0,"serving":180,"category":"diet_fitness"},
    "프로틴피자": {"cal":180,"prot":18.0,"carbs":15.0,"fat":6.0,"fiber":2.0,"sodium":400,"sugar":2.0,"serving":150,"category":"diet_fitness"},

    # franchise - 편의점/배달 추가
    "편의점컵라면": {"cal":440,"prot":9.0,"carbs":62.0,"fat":17.0,"fiber":2.0,"sodium":1800,"sugar":4.0,"serving":65,"category":"franchise"},
    "편의점주먹밥": {"cal":165,"prot":4.0,"carbs":30.0,"fat":3.0,"fiber":0.5,"sodium":350,"sugar":1.5,"serving":100,"category":"franchise"},
    "편의점핫도그": {"cal":260,"prot":8.0,"carbs":25.0,"fat":14.0,"fiber":1.0,"sodium":550,"sugar":4.0,"serving":100,"category":"franchise"},
    "편의점떡볶이": {"cal":170,"prot":4.0,"carbs":30.0,"fat":4.0,"fiber":1.0,"sodium":500,"sugar":6.0,"serving":200,"category":"franchise"},
    "편의점만두": {"cal":200,"prot":7.0,"carbs":24.0,"fat":8.0,"fiber":1.0,"sodium":450,"sugar":2.0,"serving":120,"category":"franchise"},
    "편의점유부초밥": {"cal":155,"prot":4.0,"carbs":26.0,"fat":4.0,"fiber":0.5,"sodium":380,"sugar":5.0,"serving":150,"category":"franchise"},
    "편의점참치마요": {"cal":175,"prot":5.5,"carbs":28.0,"fat":5.0,"fiber":0.5,"sodium":400,"sugar":2.0,"serving":120,"category":"franchise"},

    # korean - 건강한 한식 보완
    "닭가슴살미역국": {"cal":25,"prot":3.5,"carbs":2.0,"fat":0.5,"fiber":0.5,"sodium":320,"sugar":0.3,"serving":400,"category":"korean"},
    "소고기미역국": {"cal":30,"prot":3.5,"carbs":2.0,"fat":1.0,"fiber":0.5,"sodium":350,"sugar":0.3,"serving":400,"category":"korean"},
    "콩비지찌개": {"cal":45,"prot":3.5,"carbs":3.5,"fat":2.0,"fiber":2.0,"sodium":350,"sugar":0.5,"serving":300,"category":"korean"},
    "청국장": {"cal":60,"prot":5.0,"carbs":4.0,"fat":3.0,"fiber":2.0,"sodium":400,"sugar":1.0,"serving":300,"category":"korean"},
    "묵사발": {"cal":30,"prot":1.0,"carbs":6.0,"fat":0.2,"fiber":0.5,"sodium":300,"sugar":1.0,"serving":400,"category":"korean"},
    "물냉면": {"cal":90,"prot":4.0,"carbs":18.0,"fat":0.5,"fiber":1.0,"sodium":400,"sugar":3.0,"serving":500,"category":"korean"},
    "회냉면": {"cal":100,"prot":4.5,"carbs":18.0,"fat":1.5,"fiber":1.0,"sodium":420,"sugar":4.0,"serving":500,"category":"korean"},
    "막국수": {"cal":95,"prot":4.0,"carbs":19.0,"fat":0.5,"fiber":1.5,"sodium":380,"sugar":3.0,"serving":400,"category":"korean"},
    "도토리묵밥": {"cal":60,"prot":2.0,"carbs":12.0,"fat":0.5,"fiber":1.5,"sodium":350,"sugar":1.5,"serving":400,"category":"korean"},
    "비빔쌈밥": {"cal":105,"prot":4.5,"carbs":17.0,"fat":2.0,"fiber":2.5,"sodium":300,"sugar":1.5,"serving":300,"category":"korean"},

    # snack_drink - 건강음료/간식 마무리
    "녹즙": {"cal":20,"prot":1.0,"carbs":3.5,"fat":0.2,"fiber":1.5,"sodium":10,"sugar":1.5,"serving":200,"category":"snack_drink"},
    "ABC주스": {"cal":35,"prot":0.5,"carbs":8.0,"fat":0.1,"fiber":1.0,"sodium":15,"sugar":6.0,"serving":200,"category":"snack_drink"},
    "레몬물": {"cal":5,"prot":0.1,"carbs":1.5,"fat":0,"fiber":0.1,"sodium":2,"sugar":0.5,"serving":500,"category":"snack_drink"},
    "애플사이다비니거": {"cal":22,"prot":0,"carbs":0.9,"fat":0,"fiber":0,"sodium":5,"sugar":0.4,"serving":15,"category":"snack_drink"},
    "프로틴도넛": {"cal":280,"prot":15.0,"carbs":30.0,"fat":12.0,"fiber":3.0,"sodium":250,"sugar":8.0,"serving":60,"category":"snack_drink"},
    "프로틴칩": {"cal":400,"prot":30.0,"carbs":38.0,"fat":15.0,"fiber":3.0,"sodium":580,"sugar":2.0,"serving":30,"category":"snack_drink"},
    "무설탕젤리": {"cal":10,"prot":1.5,"carbs":1.0,"fat":0,"fiber":0,"sodium":20,"sugar":0,"serving":100,"category":"snack_drink"},
    "단백질시리얼": {"cal":370,"prot":25.0,"carbs":50.0,"fat":7.0,"fiber":5.0,"sodium":250,"sugar":5.0,"serving":40,"category":"snack_drink"},
    "저칼로리떡": {"cal":150,"prot":3.0,"carbs":35.0,"fat":0.3,"fiber":2.0,"sodium":100,"sugar":5.0,"serving":50,"category":"snack_drink"},

    # ── 1000종 달성 마무리 ──
    "프로틴또띠아": {"cal":180,"prot":15.0,"carbs":20.0,"fat":5.0,"fiber":5.0,"sodium":350,"sugar":1.0,"serving":60,"category":"diet_fitness"},
    "에그화이트오믈렛": {"cal":55,"prot":11.0,"carbs":0.7,"fat":0.2,"fiber":0,"sodium":170,"sugar":0.5,"serving":120,"category":"diet_fitness"},
    "저지방소시지": {"cal":130,"prot":16.0,"carbs":3.0,"fat":6.0,"fiber":0,"sodium":500,"sugar":1.0,"serving":50,"category":"diet_fitness"},
    "저탄수식빵": {"cal":190,"prot":13.0,"carbs":10.0,"fat":11.0,"fiber":7.0,"sodium":350,"sugar":1.5,"serving":40,"category":"diet_fitness"},
    "냉동닭가슴살": {"cal":110,"prot":23.0,"carbs":1.0,"fat":1.5,"fiber":0,"sodium":400,"sugar":0.5,"serving":100,"category":"diet_fitness"},
    "제로슈가시럽": {"cal":0,"prot":0,"carbs":0,"fat":0,"fiber":0,"sodium":5,"sugar":0,"serving":15,"category":"diet_fitness"},
    "맛살": {"cal":95,"prot":7.0,"carbs":14.0,"fat":1.0,"fiber":0,"sodium":650,"sugar":5.0,"serving":50,"category":"korean"},
    "비엔나소시지": {"cal":280,"prot":12.0,"carbs":3.0,"fat":24.0,"fiber":0,"sodium":800,"sugar":1.5,"serving":50,"category":"korean"},
    "스팸": {"cal":260,"prot":13.0,"carbs":2.0,"fat":22.0,"fiber":0,"sodium":1100,"sugar":0.5,"serving":50,"category":"korean"},
    "저염스팸": {"cal":245,"prot":14.0,"carbs":2.0,"fat":20.0,"fiber":0,"sodium":700,"sugar":0.5,"serving":50,"category":"korean"},
}


# ── AI Hub 데이터셋 통합 (2026-05-13) ──
# core_extension.json: AI Hub 400종 한식·외식 영양 메타데이터에서 추출
#   - 영양소 보강: 138건 (정확매칭 130 + 부분매칭Y 8) — CORE 영양값 유지 + 7개 영양소(칼슘·인·칼륨·마그네슘·철·아연·콜레스테롤) 추가
#   - 신규 추가:   262건 (부분매칭N 161 + AI Hub 단독 101) — CORE에 없던 한식 추가
# 출처: AI Hub 음식분류 AI 데이터 영양DB v1.0 (㈜에이아이더 + 뉴트리진 + ㈜어메이징푸드솔루션)
try:
    _ext_path = Path(__file__).parent.parent / 'core_extension.json'
    if _ext_path.exists():
        with open(_ext_path, 'r', encoding='utf-8') as _f:
            _ext_data = json.load(_f)
        # 1) 영양소 보강 (기존 CORE 항목에 7개 영양소만 추가, 기존 영양값은 유지)
        _NUTRIENT_KEYS = ('calcium', 'phos', 'pot', 'mag', 'iron', 'zinc', 'chol')
        _enhanced_n = 0
        for _nm, _data in _ext_data.get('enhanced', {}).items():
            if _nm in CORE_FOODS:
                CORE_FOODS[_nm].update({k: _data[k] for k in _NUTRIENT_KEYS if k in _data})
                _enhanced_n += 1
        # 2) 신규 추가 (CORE에 없는 한식)
        _new_n = 0
        for _nm, _data in _ext_data.get('new_additions', {}).items():
            if _nm not in CORE_FOODS:
                CORE_FOODS[_nm] = _data
                _new_n += 1
        print(f"[CORE 확장] AI Hub 데이터 로드 완료: 보강 {_enhanced_n}건 + 신규 {_new_n}건 → CORE_FOODS 총 {len(CORE_FOODS)}건")
...
def match_with_db(analysis, foods_db):
    ...
    for food in analysis["foods"]:
        ...
        if gold_data:
            ...
            # ── 칼로리 정확도 개선 (2026-05-06): 카테고리별 차등 보정 ──
            # Gemini 자문 반영: piece 음식은 AI 100% 신뢰 (다이어터의 1/4 사과를 표준 강제 X),
            # volume·drink 음식만 AI 양이 표준에서 크게 벗어나면 가중 평균으로 보정.
            shape = _detect_shape(ai_name, gold_data.get('category'))
            food['shape'] = shape  # 디버깅 / 시트 백업용 메타데이터

            if ai_serving:
                if shape == 'piece':
                    # 개수·조각 음식 (사과·햄버거·피자·도넛 등): AI 양 그대로 신뢰
                    # 사용자가 1/4쪽 먹은 사과를 표준 1개로 강제하지 않음
                    effective_serving = ai_serving
                elif shape in ('volume', 'drink') and gold_data.get('source') == 'core' and gold_serving > 0:
                    # 무정형/음료: AI 양이 CORE 표준에서 너무 벗어나면 가중 평균
                    # 부드러운 보정 — 강제 덮어쓰기는 "평균으로의 회귀" 부작용 우려
                    diff_ratio = ai_serving / gold_serving
                    if diff_ratio > 2.5 or diff_ratio < 0.4:
                        # 명백한 빗나감 — CORE 표준 사용
                        print(f"[양 보정] {ai_name} ({shape}): GPT {ai_serving}g → "
                              f"CORE 표준 {gold_serving}g (비율 {diff_ratio:.1f}배)")
                        effective_serving = gold_serving
                    elif diff_ratio > 1.5 or diff_ratio < 0.7:
                        # 의심스러움 — GPT 70% + CORE 30% 가중 평균
                        blended = round(ai_serving * 0.7 + gold_serving * 0.3)
                        print(f"[양 가중] {ai_name} ({shape}): GPT {ai_serving}g + "
                              f"CORE {gold_serving}g → {blended}g")
                        effective_serving = blended
                    else:
                        effective_serving = ai_serving
                else:
                    effective_serving = ai_serving
